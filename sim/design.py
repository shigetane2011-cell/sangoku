...
TRAIT_PRICE = {
    ...
    'mochi': 1.6734,
    'vanguard': 0.7370,
    # 庇護（§7.144・テストプレイの提案）。両隣の前衛が受ける矢の3割を代わりに
    # 受ける常在型。味方依存（隣に誰がいるかで値が変わる）なので本陣と同じく
    # 勝率の通貨（4.86 勝点%/コスト点）で測った。土台10種の**単純平均**
    # （テストプレイ「有効な使い方をした時の最大値ではなくアベレージで」）:
    #   隣が10コスト騎兵（赤壁40）+0.99/+1.85/+1.23/+1.03、端の庇い手+呂布 +1.71、
    #   隣が中低コスト騎兵 +0.13（官渡）/+0.39（官渡）/+0.77（汜水関）、
    #   **隣が歩兵は負** −0.69（張飛）/−0.90（歩兵の中央）— 丈夫な隣の矢まで
    #   引き受けて庇い手が先に倒れる。中央値 0.88・平均 0.652。
    'cover': 0.652,
    'vs_wei': 0.4284,
    'vs_shu': 0.4284,
    'vs_go': 0.4284,
