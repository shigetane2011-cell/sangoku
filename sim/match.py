...
#
# 【§7.236】汜水関 18 → 20（テストプレイの決定「デッキに入れる武将を悩む
# 選択肢が狭い」）。測定記録は docs/balance/experiments/deck-breadth-20260910.md。
# 6枠なので最低6点は必ず要る。**自由に使える点が 12 → 14**（+17%）で、
# 章の坂（12→24→34）が 14→24→34 になり3章→4章の跳ねが緩む。
# 測って分かったのは、狭さが**予算のせいで札の強弱の差のせいではない**こと:
#   ・上限18は合計が常に上限ちょうどなので、1枚差し替えが必ず同額か格下になる
#     （「より高い札へ」の手が 0%。20点で41%）
#   ・「全6枚を3点以上」にできるコストの形が18点だと1通りだけ（3,3,3,3,3,3）
#   ・各枠の上位6枚のコスト帯が 1〜3点中心（1コスト札が38/108）から
#     **4〜5点中心**（1コスト札が6/108）へ移る
#   ・「最良から1点以内が4枚以下」の枠が 6/18 → 3/18 へ半減
#   ・枠の**中央値は10枚のまま動かない** ＝ 動くのは裾と中心だけ
# 24点は幅がほとんど増えない（好手 +25%→+30%）のに上位の80%が7点以上になり、
# 汜水関が赤壁と同じ顔になるので採らない。
...
# レギュレーションの名は戦役から（§7.43。規模が名前だけで伝わる:
# 汜水関の小競り合い → 官渡 → 赤壁 → 天下）。旧名は読み替えで受ける。
REG_ALIAS = {"低コスト戦": "汜水関", "中コスト戦": "官渡", "高コスト戦": "赤壁",
             "統一(BO3)": "天下"}
UNIT_SIZE = 6

# 余剰コストの変換（§4.5 案A）。余り1点につき**その札のゲージ最大値の1%**を
# 初期ゲージへ足す。上限10%。固定量ではなく最大値に対する割合なので、消費ゲージが
# 3段（75/150/300）に分かれていても同じだけ前倒しになる。
#
# 【§4.5 後記】0.02 → 0.01（2026-08-28）。0.02 は時間給3.20時代の値で、
# §7.124 の半減が初期ゲージの時間価値を倍化させていた。拮抗したラダー実デッキ
# （種48）で余剰1点が勝率31→65%（コスト13.5点分）に化け、拮抗メタでは満額
# 5点余りへ収束する恐れがあったため、旧経済と同じ「秒数の前倒し」へ再較正。
SURPLUS_PER_COST = 0.01
SURPLUS_CAP = 0.10
