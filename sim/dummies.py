...
PERSONAS: Tuple[Persona, ...] = (
    ...
    # §7.133 で追加（拡張在野）。テストプレイ側の探索が固定編成で持っていた
    # 4型を、**性格として**常設の稽古台へ入れる。固定の18人を貼らずに性格へ
    # 直したのは、名簿や値付けが動いたときに固定編成だけが取り残されるため
    # （在野は seed_ladder が性格から組み直すので、盤面の変化に追従する）。
    #
    #   先手 … 手数型を薄く広く並べて技の往来で押す（§7.127 の密度）
    #   処刑 … 一撃の大技と知略を寄せ、削れた隊を刈る
    #   謀弩 … 弓4の後衛へ予算を寄せる射撃陣（手練れ）
    #   槍陣 … 前衛を厚くし、後衛にも槍を差して回り込みを迎え撃つ
    Persona("先手", {F.CAV: 1.6, F.INF: 1.3, F.ARC: 0.8},
            {F.BURST: 2.2, F.DPS: 1.8, F.BAL: 1.2, F.TANK: 0.7, F.SUP: 0.5},
            "鶴翼", 0.25),
    # 処刑の greed は 0.8 → 0.55（測って下げた）。高コストへ寄せすぎると
    # 埋め札が1〜2点ばかりになり、12性格の総当たりで 27.6% と従来の底
    # （鉄壁35.0%）を下回った。**「一撃で刈る」性格は札の選び方では作れない**
    # — 刈る力は「残兵力が最少」を狙う技に宿るが、生成器は兵種・役割・コスト
    # でしか選べない。稽古台として立つ濃さへ戻し、性格は役割の重みで出す。
    Persona("処刑", {F.ARC: 1.4, F.INF: 1.2, F.CAV: 1.0},
            {F.DPS: 2.4, F.BURST: 1.8, F.SUP: 1.4, F.BAL: 1.1, F.TANK: 0.7},
            "魚鱗", 0.55),
    Persona("謀弩", {F.ARC: 2.0, F.INF: 1.1, F.CAV: 0.5},
            {F.SUP: 2.2, F.DPS: 2.0, F.BAL: 1.0, F.TANK: 0.8, F.BURST: 0.6},
            "雁行", 0.5, rear_share=0.55),
    Persona("槍陣", {F.INF: 2.1, F.CAV: 0.9, F.ARC: 0.8},
            {F.TANK: 2.2, F.BAL: 1.6, F.SUP: 1.1, F.DPS: 0.8, F.BURST: 0.5},
            "魚鱗", 0.35, rear_share=0.30, rear_spear=2),
)
