...
def value(cost):
    """コストから総合値を出す。**兵力と攻撃力がそれぞれコストの1次式になるように。**

    総合値 = 兵力 × 攻撃力 であり、build_card は両者を同じ倍率 f で決めるので
    f = sqrt(総合値) になる。したがって「兵力と攻撃力をそれぞれ1次式にする」には
    **総合値を1次式の2乗にする**必要がある。

    ----------------------------------------------------------------------
    なぜ1次式の2乗なのか（v0.5 末で修正）
    ----------------------------------------------------------------------

    以前は総合値そのものを1次式にしていた。すると f = sqrt(1次式) となり、
    兵力も攻撃力も**上に凸**になる。凸な関数の和は合計コストだけの関数にならない。

    ところが戦闘が使うのは (Σ兵力) × (Σ攻撃力) である。予算は
    Σᵢ(兵力ᵢ × 攻撃力ᵢ)（1枚ごとに掛けてから足す）を数えていた。この2つは
    全員が同じ能力値のときしか一致しない。

    実測（総コスト30固定・配分だけ変える）:
      配分            予算の見立て   戦闘が使う (Σ兵力)(Σ攻撃)   実測勝率
      5/5/5/5/5/5     1.25M         7.87M (1.000)            49.5%
      7/3/5/5/5/5     1.25M         7.85M (0.997)            39.1%
      9/1/5/5/5/5     1.25M         7.78M (0.988)            17.9%
    予算は不動なのに戦闘側が 1.2% 落ち、増幅がそれを 32pt に拡大していた。

    1次式の2乗にすると、兵力も攻撃力も 1次式になる。6枠固定なので
    Σ(a + b·cᵢ) = 6a + b·Σc となり、**合計コストだけで決まる**。
    枠の基礎価値（切片 a）は残るので、コスト1の札の存在意義も失われない。
    """
    ratio = SLOT_VALUE + (1 - SLOT_VALUE) * cost / NORM_COST
    return TARGET_SCORE * ratio * ratio
