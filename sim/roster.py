...
def value(cost):
    ...
    ----------------------------------------------------------------------
    「1次式の2乗にする」案を試して戻した（v0.5 末）
    ----------------------------------------------------------------------

    総合値そのものを1次式にすると、build_card は兵力と攻撃力を同じ倍率
    f = sqrt(総合値) で決めるので、**兵力も攻撃力も上に凸**になる。
    凸な関数の和は合計コストだけの関数にならない。一方で戦闘が使うのは
    (Σ兵力)×(Σ攻撃力) のはずで、予算が数えている Σᵢ(兵力ᵢ×攻撃力ᵢ) とは
    全員が同じ能力値のときしか一致しない。だから総合値を1次式の2乗にすれば
    兵力・攻撃力とも1次式になり、Σ(a + b·cᵢ) = 6a + b·Σc で合計コストだけの
    関数になる——という理屈だった。

    **狙った量は動いた。しかし勝率は悪化した。**
      配分（総コスト30）  Σ兵力   Σ攻撃   実測勝率
      5/5/5/5/5/5       21040   374    49.5%
      9/1/5/5/5/5       21040   374     1.3%   ← 合計は完全に一致している
    Σ兵力もΣ攻撃力も完全に一致していて勝率が 49.5% と 1.3% になる。
    **つまり (Σ兵力)×(Σ攻撃力) は戦闘が使っている量ではない。** 前回それらしく
    一致したのは古い曲線の偶然だった。

    対照実験でも、この案に利点はなかった（全員 歩兵・均衡役、総コスト30固定）:
                        同じレーン内 7/3 / 9/1    レーンをまたぐ 7/3 / 9/1
      変更前（この式）    50.3% / 50.5%           10.5% /  0.2%
      1次式の2乗          49.6% / 59.2%           2.8% /  1.9%
    レーン内は変更前から平らで、9/1 はむしろ悪化した。よって戻した。

    ----------------------------------------------------------------------
    本当の問題はここではない（v0.5 末で判明）
    ----------------------------------------------------------------------

    **コストが足し算になるのはレーンの中だけである。** 総コストを固定して
    配分だけを動かすと、同一レーン内なら 50.7 / 48.7 / 49.1%（零点50.0%・
    2σ=8.2pt）で平ら。レーンをまたぐと 12.3 / 2.7 / 0.6% まで落ちる。
    前衛どうしの移動がとくに鋭い（前衛が盾なので、薄くしたレーンが崩れる）。

    **勘定の単位は部隊ではなくレーンである。** 能力値の曲線をどういじっても
    ここは直らない。§13 へ。
    """
