...
def check() -> int:
    ...
    **CSV の中だけで閉じた検算**だった。命中率とクリ率を掛けた総合値は盤面に
    ...
    # --- 移動速度が「接敵前」に飛ぶ札の見張り（§7.230） --------------------
    # `EFFECT_PRICE["spd"] = 0.0` は**接敵後なら実測 0** だから置けている値である。
    # 接敵は中央値 5.2秒。**初期ゲージ ≧ 消費ゲージ なら開幕（0.2秒）に飛ぶ**ので、
    # そこへ移動速度を載せると **0.6〜1.6 コスト点をタダで**持っていく（§7.230 の実測）。
    # いまの名簿には1枚も無いが、**足したときに黙って通ると危ない**ので鳴らす。
    # 止めはしない（警告のみ）— 値付けを変える判断はここではできない。
    early = []
    for g in G:
        sk = F.SKILL_INFO.get(g["兵法"])
        if sk is None or not any(k == "spd" for k, _a, _s in sk.mods + sk.self_mods):
            continue
        if float(g["初期ゲージ"]) >= float(g["消費ゲージ%"]):
            early.append((g["名前"], g["兵法"]))
    if early:
        print("  ⚠️ 移動速度を**開幕に**飛ばす札（値札0のまま 0.6〜1.6点ぶんタダ・§7.230）: {}".format(
            "・".join("{}〔{}〕".format(n, s_) for n, s_ in early)))
...
COLUMNS = ["名前", "人物", "字号", "勢力", "コスト", "帯", "兵種", "役割",
           ...
           # 総合力（§7.247）。**床調整ぶんを足し戻したコスト点。** 効果予算 +
           # 能力値コスト は作りかたから必ずコストに一致するので、床調整
           # （総合値を ±30% 動かす）はシートのどこにも出ていなかった。
           # 「この札は実は何点ぶんの身体を持っているか」を読める列にする。
           "総合力",
...
def load_traits_into_field() -> int:
    ...
    for t in traits():
        if t["型"] != "誘発":
            ...
            #   増幅   ＜的の状態＞の敵への＜損害の種類＞ +N%   … 軍全体
            #   自己増幅 ＜同上＞                              … 持ち手だけ（§7.212）
            # 残りの常在（陣頭・本陣・馬前…）は従来どおり field.py 側で個別に扱う。
            m = re.match(r"(自己)?増幅\s*(\S+?)の敵への(\S+?)\s*\+(\d+)%",
                         (t["効果"] or "").strip())
            if m:
                cond = F.AMP_COND_JP.get(m.group(2))
                kind = F.AMP_KIND_JP.get(m.group(3))
                if cond is None or kind is None:
                    raise SystemExit(
                        "traits.csv: 増幅の語彙が未定義 — {}（条件 {} / 損害 {}）\n"
                        "  条件は {} ／ 損害は {} のどれか".format(
                            t["キー"], m.group(2), m.group(3),
                            "・".join(F.AMP_COND_JP), "・".join(F.AMP_KIND_JP)))
                F.AMPLIFY[t["キー"]] = (cond, kind, float(m.group(4)) / 100.0,
                                        "self" if m.group(1) else "army")
            continue
        note = t["備考"]
