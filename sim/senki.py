...
設計の決めごと（docs/design/senki-campaign-draft.md）:
  - 帯の進行: 1〜3章＝汜水関20 → 4〜5章＝官渡30 → 6〜8章＝赤壁40。
...
#
# **ただし固定の1点は帯によって重みが違う**（§7.109）。赤壁の40点なら 2.5% の
# ハンデだが、第1戦の8点では 12.5% ——**5倍きつい**。上の較正は全戦の平均で
# 合わせたので、大きい帯に寄って序盤が潰れていた（第1戦は草案5通りとも 0%、
# 第2戦は最良 22%）。
#
# 割合で持ち、**整数へ丸める**。0.025 なら 敵20点以上で従来どおり1点、
# それ未満（序盤）は 0点＝敵と同じ重さになる。上の較正は大きい帯で効くので
# 壊れない。序盤は「同じ重さで、編成の巧さだけで勝つ」形になる。
SENKI_EDGE_FRAC = 0.025

# 周回スケーリング（§7.60）: **2周に1点、家来を強い実カードへ差し替える。**
#   周回Nの敵 ＝ 章ボスのデッキの雑兵を（大将と登用対象は固定のまま）
#   同兵種・+1点の実在カードへ順に入れ替えたもの（合計 素+⌊N/2⌋点）。
# 差し替えなら**兵法ごと強くなる**。ここに至る棄却の履歴:
#   1) +N点を cost へ等分 → コスト曲線は兵力に平らで兵法が買えず、雑兵構成の
#      ボスは+30点でも脅威にならなかった（実測）。
#   2) 兵力×(1+1.2%N) の乗算 → 効くが、毎周同じ敵の数字が増えるだけで、
#      兵法の穴も残る。差し替え式は毎周顔ぶれが変わり、パズルが更新される。
...
def enemy_cost(cards, b: Dict) -> float:
    ...
    **全52戦では悪化した**（難所 14戦 → 16戦）。`suggest_deck` の組み方が上限に
    ...
    【訂正・§7.116】長駆の斧の 0% は床調整が主因ではなかった。草案が敵の
    陣形を鏡写しして自滅していたためで、巡回に直すと 100% になる。床調整が
    敵に集まる戦への警戒自体は残す（最後のひと押しにはなる）が、難所が出たら
    まず**草案の側**（陣形・引き直し）を疑うこと。
    """
...
def lap_fight(cx, cards, me, now: int) -> Dict:
    ...
    周回Nの敵＝lap_enemy（家来の差し替え＝兵法ごと強くなる）。勝てば残兵
