...
TROOP_ADVANTAGE = 103         # 兵種有利のダメージ補正 +3%（§5.3・v0.5 で再較正）
# 部隊戦は60秒以上続くため、毎回の攻撃に乗る補正は複利で効く。+15% では
# コスト揃えの単一兵種編成で有利側が100%勝利していた。実測は
# +3%→47/100/64、+5%→72/100/76、+10%→95/100/95（歩→騎/騎→弓/弓→歩）。
#
# 104 から 103 へ下げたのは、後衛が前衛の後方に留まるようになり（hold_line）、
# 迂回が位置まで届けるようになった（flank_step）ぶん三すくみが鋭くなったため。
# 104 のままだと 65/71/73 で弓兵→歩兵が上端を超える。103 で 61/65/67。
# **三本まとめて 50 へ寄せる札はこれしかない。** 弓兵を弱める札では
# 弓兵→歩兵 が下がる代わりに 騎兵→弓兵 が上がり、二つの帯外が逆へ動く。
...
class Battle:
    ...
    def reach(self, u: Unit) -> int:
        r = self.base(u, "range")
        # **後衛は前衛越しに戦う。留まるぶんだけ射程が伸びる（§5.2）。**
        #
        # 後衛が前衛の後方15単位に留まると、そのままでは一発も攻撃できない。
        # 両軍の前衛は互いの射程Rで止まるので、自軍後衛から敵前衛までは常に
        # **R + 15単位**になり、射程を一律に上げてもRが増えるだけで差は縮まらない。
        # 縮めるには後衛だけが長く届く必要がある。二列目が槍で戦う形である。
        #
        # 伸ばす量はちょうど食い違いのぶん（BACK_OFFSET）。前へ出ない代わりに
        # 同じ距離へ届くので、位置は仕様どおり後方のまま、戦闘への寄与は変わらない。
        # 自軍前衛が全滅したレーンでは前へ出るので、この延長は付かない。
        if u.row == "back" and any(m.row == "front"
                                   for m in self.alive(u.side, u.lane)):
            r += BACK_OFFSET
    ...
    def flank_step(self, u: Unit, enemies: list) -> bool:
        ...
        if u.detour <= 0:
            u.detour = 0
            u.flanked = True
            # **回り込みが終わったら位置も敵前衛の裏へ移す。**
            #
            # 以前は迂回が時間だけで、位置は動かなかった。それでも成立していたのは
            # 後衛が自軍前衛の位置まで詰めていて、両軍の4列が100単位の中に
            # 固まっていたからである。回り込み終了時点で敵後衛はすでに射程内にいた。
            #
            # 後衛を後方150に留めると敵後衛までが400になり、回り込むたびに
            # 150ぶん余計に歩くことになる。騎兵の勝ち筋は総大将のいる敵後衛を
            # 潰すことなので、この150がそのまま勝率に出た。実測では敵後衛の
            # 撤退率が 10%→6% に落ち、歩兵→騎兵が 68%→88% へ跳ねている。
            # 「詰め寄る時間 0.6秒ぶん」でも「迂回後の 1.25秒ぶん」でもなく、
            # **勝ち筋そのものが遠くなったこと**が20ptの正体だった。
            #
            # 回り込みとは敵前衛の裏へ出ることなのだから、出た先へ置く。
            # 変更前の状態を復元するだけで、迂回を強くするものではない。
            tgt = min(back, key=lambda e: abs(e.pos - u.pos))
            r = self.base(u, "range")
            u.pos = tgt.pos - r if u.side == 0 else tgt.pos + r
    ...
    def hold_line(self, u: Unit, mv: int) -> int:
        """後衛は自軍前衛の後方 BACK_OFFSET より前へ出ない（§5.2）。

        **単独では入れられない。三つで一組である。**
          1. これだけ → 三すくみ 100/0/100。近接の後衛は射程100に対し敵まで
             250離れるので一発も攻撃できず、6枚のうち3枚が遊ぶ。
          2. 射程延長（reach）と対にする → 88/56/72。攻撃はできるが、
             騎兵の勝ち筋である敵後衛が150遠のく。
          3. 迂回で裏へ出る（flank_step）まで入れて 65/71/73、
             兵種有利を 104→103 にして 61/65/67。

        分解の実測（同じ種で対応させた比較）:
          留まるだけ 100/ 0/100   延長だけ 88/56/59   両方 88/56/72
        射程延長は「敵前衛の250手前で止まる」ことを意味するので、
        延長だけでも留まるのとほぼ同じ位置になる。二つは別の札ではない。

        前衛が全滅したレーンでは制限しない（§5.2「前衛が撤退すると後衛が
        前衛位置へ前進する」）。迂回済みの騎兵は flank_step が敵前衛の裏へ
        出しており、この線より前にいるので mv=0 になるが、そこが射程内なので
        困らない。**外しても勝率は動かなかった**（65/72/72 のまま）ので入れない。
        """
        if u.row != "back":
            return mv
        fronts = [m for m in self.alive(u.side, u.lane) if m.row == "front"]
        if not fronts:
            return mv
        if u.side == 0:
            return max(0, min(mv, min(m.pos for m in fronts) - BACK_OFFSET - u.pos))
        return max(0, min(mv, u.pos - max(m.pos for m in fronts) - BACK_OFFSET))

    # 効かなかった案の記録（同じ所へ戻らないために残す）。
    #
    # ・迂回済みを hold_line と射程延長の両方から外す → 88→86。ほぼ動かない。
    # ・迂回の開始判定を素の射程にする（延長を除く）→ 100/0/100。後衛の騎兵は
    #   留まる線より内側へ入れないので**永久に迂回を始められなくなる**。
    # ・後衛の布陣を 2×BACK_OFFSET 深くして「詰め寄る時間」を返す
    #   → 87/9/91。弓兵の後退限界（home）まで深くなり、後退射撃が伸びる。
    # ・孤立の罰 FLANKED_TAKEN を下げる → 200 で 88%、160 で 2%。二値の札。
    # ・突撃 CHARGE_BONUS を上げる → 30/60/90/130 で 88/85/81/81。頭打ち。
    #
    # これらを潰す過程で「詰め寄る時間 0.6秒」も「迂回後の余分な 1.25秒」も
    # 20pt を説明できないことが分かり、**行ごとの撤退率**を見て
    # 「敵後衛が遠くなった」＝勝ち筋が遠くなったことが原因だと特定できた。
    # 位置や時間の差ではなく、**何を落として勝つ編成なのか**から見るのが早い。
    #
    # 「後衛有利を受け入れ、例外を特性で作る」は §4.2 で確定済みの方針である。
    ...
    def step(self):
        ...
        for u in actors:
            ...
            if not self.alive(1 - u.side, u.lane):
                ...
                #
                # **「隣に敵がいるか」ではなく「射程内にいるか」で判定する。**
                # 存在するだけで留めていたため、届かない相手を眺めたまま
                # 支援移動もしない弓兵が出ていた。実測では、後退射撃で自陣の
                # 端まで下がった黄忠（射程450）が隣レーンの敵まで717あるのに
                # 動かず、**5.7秒のあいだ一発も撃たずに立っていた**。
                # 決着間際だったので勝敗は変わらなかったが、レーンが片付いた側の
                # 兵力が丸ごと遊ぶ形で、盤外へ歩き去っていた不具合と同じ型である。
                #
                # 届かないなら支援移動へ回す。移動は速度割りなので（lane_support）、
                # ここでも足の速さが効く。
                if not any(abs(e.pos - u.pos) <= self.reach(u)
                           for e in self.cross_lane_foes(u)):
                    self.lane_support(u)
                continue
            ...
            if gap > self.reach(u):
                ...
                mv = self.hold_line(u, mv)
                # **標的のいる向きへ進む。側で向きを固定してはいけない。**
                # 固定していたときは、敵に追い越されると永久に離れていった。
                # 弓兵を追う味方が自分を追い越すと標的が背後に回り、そこから
                # 「近づくために」逆向きへ歩き続ける。実測では1戦あたり2.8人が
                # 盤外へ出ており、実カードでは盤面の幅130単位に対し852単位も
                # はみ出していた。兵力を丸ごと残したまま戦線を離れる。
                # 合成の単一兵種編成では全員同速で追い越しが起きないため、
                # 三すくみの測定には一度も現れなかった。
                u.pos += mv if target.pos > u.pos else -mv
