...
def _skill_display(g, sk_row, scaled: bool = True) -> str:
    ...
    # 【§7.258】勢力で縛る号令。**届く人数が編成しだい**だという肝心のところが
...
def _trait_brief(g, key, t):
    ...
    if key == "vanguard":
        ...
        # 馬前（§7.144）。隣＝同じ前衛列の両隣の騎兵だけ。庇う側の鎧で受け直す
        desc = ("前衛に置くと、両隣の前衛の騎兵が受ける矢の {:.0%} を代わりに受ける"
...
def _treasure_brief(key, row):
    ...
    if key == "t_toko":
        ...
        return ("この将の決めゼリフが必ず実況に出る。"
                "張飛は酔って徐州の城門を開かれ、呂布は酒を禁じて下邳で部下に売られた")
    return row.get("備考", "")
