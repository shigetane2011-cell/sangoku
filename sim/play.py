...
def treasure_budget_kou(cap: float) -> int:
    """軍功予算（戦場比例 cap×3功 — 汜水関60・官渡90・赤壁120。§7.140）。
    （汜水関は §7.236 で上限 18→20 になったので 54→60 功）

    宝物のセットはデッキ本体の点でなく**専用の別予算**を消費する（「ご褒美を
    付けたら編成が崩れる」体験を避ける）。単位は「功」＝0.01コスト点の整数
    （表示に小数点を出さない）。全員一律なので、古参と新規の差は強さでなく
    品揃え（選択肢の数）だけになる（§3.1: 在席時間を強さにしない）。

    倍率は §7.61 の cap×5 から §7.140 で cap×3 へ締めた: 値付けの実測
    （§7.139）で予算の購買力が3倍に膨らみ、しかも宝物の価値は束ねても
    目減りしない（§7.140 の積み増し検算）ので、予算がそのまま「フル装備の
    古参 vs 丸腰の新規」の勝率差の天井になる。×3 で天井は汜水関+2.6%・
    官渡+4.4%・赤壁+5.8%。最高額の宝（青龍偃月刀110功）は赤壁でだけ
    担げる — 死に宝は出ない（120 ≥ 110 が下限の制約）。"""
    return int(round(cap * 3))
...
TREASURE_CARD_MODS: Dict[str, Dict[str, float]] = {
    ...
    # 【§7.203】実測で値段が付かなかった札の作り替え。
    #   白羽扇  知力+15 ＝5功 → **+30**（知力は知略の兵法と混乱耐えにしか乗らない）
    #   玄鉄/軽騎 寄せ ±0.3 ＝7功/0功 → **±0.6**（寄せは総合値を保つ付け替えなので
    #           設計上ほぼ0が正しい。量を増やして初めて意味が出る）
    #   赤兎馬  兵力だけ ＝3功 → **兵力+4% と武力+10 に分ける**（テストプレイの指示）
...
def tick(cx, cards, now: int) -> None:
    ...
    #      全部持ちの状態は試験用ボタン（dev_senki / 当時 dev_onsho・現
    #      dev_treasure）で1押しで
...
def print_report(ua, ub, dt: float, seed: int, me_first: bool) -> None:
    ...
    for tag, rows in (("自軍", mine), ("敵軍", foe)):
        ...
        for (name, typ, dealt, men, men0, sd, _fa,
             ff, rf, cs, hl, al, *_detail, wp, _cv) in rows:
