...
- 戦列は広がらない。開始 1000.0m → 終了 1000.0m。dt を 8倍振っても、
  時間を 8倍に延ばしても変化 0.0m。
- コストの加算性: 9/1 まで偏らせても総コスト30点中 0.37%（dt 不変）。
- 兵種の三すくみ: 3方向が ±0.02 コスト点で揃っている（§2）。
...
================================================================================
 2. 兵種の三すくみ（立った。`field.py price` → `field.py triangle`）
================================================================================
■ 止めていたのは兵種補正ではなく **行動面の係数** だった。
   §6.1 の係数（歩1.00 / 騎0.90 / 弓1.12）は旧レーンエンジンで測った値である。
   これを入れたまま兵種補正を 0 → 0.15、FOCUS を 0 → 3 まで振っても、三すくみは
   0 / 100 / 0 から**一切動かなかった**（同兵種は 50.0 なので計器は生きている）。
   係数だけで勝敗が決まっていて、他のつまみが全部効いていない状態だった。

■ **勝率では測れないと分かった。これが最大の発見である。**
   総コストの優位と勝率の対応を較正すると、

       追加コスト   +0.00   +0.05   +0.10   +0.30
       勝率          50.0   100.0   100.0   100.0

   **総コスト 0.1点（30点中 0.3%）で勝率が 100% になる。** 決定論の盤面では、
   わずかでも強い側がパネルの全マスで勝つので、勝率は 0 / 50 / 100 の階段にしか
   ならない。中間の値が出るのは「マスによって勝つ側が入れ替わるとき」だけで、
   これは強さの大きさではなく**符号のばらつき**を測っている。
   したがって §5.3 の目標帯「55〜80%」はこの計器では踏めない。**帯そのものが
   踏めないのであって、調整が下手なのではない。** 以後はコスト点で測る。

■ トーナメント表を **推移成分**（単にどれかが強い）と **巡回成分**（本物の
   三すくみ）へ分解した。**推移成分だけが値付けで消せる。** 分けずに勝率だけを
   見ていると「三すくみが立たない」のか「1つが強いだけ」なのかが区別できない。
   実測すると、補正 0 での偏りはほとんどが**単なる値付けのずれ**だった
   （三すくみが壊れていたのではない）。

■ 3兵種の総当たりで値付けし直した（`field.py price`、rot=6 の全域）。歩兵基準の
   1本だけで合わせると残り1組のずれが取れないため、総当たりの推移成分を係数へ
   戻す。2回で収束した。

       反復0: 歩 +0.515  騎 +1.968  弓 -2.483
       反復2: 歩 +0.005  騎 +0.011  弓 -0.016   → 歩1.0000 / 騎1.0459 / 弓0.9105

■ 接敵抑制を入れた（§5.3「騎兵→弓兵: 高速接近により遠距離攻撃を封じる」）。
   これがないと弓兵は密着されても撃ち続け、騎兵→弓兵が動かない。ただし**効きが
   鋭い**。抑制 0.01 につき弓兵の値付けが約 0.6% 動くので 0.15 に留め、残りは
   係数で吸収した。

■ 結果（rot=6 の全域、単位はコスト点、総コスト30に対して）。

       兵種補正   歩→騎   騎→弓   弓→歩    巡回c    推移の幅
       0.000     -0.094  -0.065  -0.109  -0.0895    0.024
       0.020     +0.228  +0.251  +0.215  +0.2311    0.020
       0.040     +0.546  +0.564  +0.536  +0.5488    0.016
       0.060     +0.861  +0.875  +0.855  +0.8635    0.011

   **3方向が ±0.02 コスト点で揃った。** 推移の幅がほぼ 0 なので「単に強い兵種」は
   居ない。巡回成分は補正にきれいに比例する（c ≒ -0.09 + 15.9 × 補正）。
   時間刻みでも動かない（dt=1.0 / 0.5 / 0.25 で揃う）。パネルの大きさにも
   よらない（rot=2..6 で 0.48〜0.57）。

   §5.3 の +4% を採ると、有利側の優位は **0.55 コスト点 ＝ 総コストの 1.8%**。
   カード1枚（コスト5）より十分小さく、編成で覆せる範囲にある。

■ 補正 0 の地点で巡回が **-0.09**（素の幾何は 騎→歩→弓→騎 と逆回り）である。
   補正はまずこれを打ち消してから効くので、補正 0.006 あたりが実質の零点になる。

■ **仕様への提案**: §5.3 の目標帯を勝率（55〜80%）ではなく**コスト点**で書き直す
   こと。暫定案「有利側の優位を総コストの 1〜3%（30点中 0.3〜0.9点）に収める」。

================================================================================
 3. 陣形（測れた。極端どころか、札の置き方より弱い）
================================================================================
面積 500×30 を保存したまま正面と奥行を交換する形で表した。正面を広げれば奥行が
薄くなる。単位はコスト点、rot=6（全36通り）。括弧内はマスごとのばらつき。

  札の組        標準 対 広く浅い      標準 対 狭く深い    広く浅い 対 狭く深い
  標準混成      -0.099 (±0.77)     -0.452 (±0.98)    -0.078 (±1.65)
  歩兵寄り      -0.118 (±0.50)     -0.557 (±0.76)    -0.026 (±0.96)
  騎兵寄り      -0.107 (±0.76)     -0.411 (±1.04)    -0.130 (±1.61)
  弓兵寄り      -0.162 (±0.84)     -0.390 (±0.87)    -0.070 (±1.31)

- **順位は札の組を変えても崩れない。** 4つの組すべてで符号が同じで、
  **狭く深い > 広く浅い > 標準**。推移の幅 0.36 / 巡回 0.09 コスト点なので、
  循環はわずかにあるが主成分は「狭く深いが単に強い」である。
- **差そのものは小さい。** 最大でも 0.45 コスト点＝総コストの 1.5%。旧エンジンで
  「陣形の相性だけで札の質が帳消しになる（0%/100%）」と見えていたのは、
  §13 に記録した**勝率の飽和**であって、モデルの極端さではなかった可能性が高い。
- **むしろ逆の問題がある。** マスごとのばらつき（±0.5〜1.65）が陣形間の差
  （0.03〜0.45）の **5〜15倍**ある。つまり **どの陣形を選ぶかより、どの札をどこへ
  置くかのほうが桁違いに効く。** 陣形は戦術の軸としては現状ほぼ効いていない。
  「極端でない差になるか」への答えは「なる。ただし弱すぎる側に振れている」。

================================================================================
 残課題
================================================================================
 - **陣形を戦術の軸として意味のある強さにする。** いまは札の置き方に埋もれている
   （上の 3）。効果量を上げるか、陣形が置き方の自由度そのものを変える形にするか。
 - §5.3 の目標帯を勝率からコスト点へ書き直す（仕様側の変更）。
 ...
 - **このファイルは引き継ぎ時点の field.py が1つもコミットされていなかったため、
   引き継ぎ文書に書かれた寸法（1枚10,000人・正面500m×奥行30m・第二線120m後方・
   弓の射程180m・盤面1580m×300m）から組み直したものである。** 数値は前任の
   実装と一致しない。零点・時間刻み・加算性はすべてこのファイルで測り直している。
"""
...
ACT_COEF = {INF: 1.0000, CAV: 1.0459, ARC: 0.9105}
