...
ACT_COEF = {INF: 1.0000, CAV: 1.0614, ARC: 0.7836}
...
TYPE_EDGE_COST = {(INF, CAV): 1.0484, (CAV, ARC): 2.4979, (ARC, INF): 2.4951}
...
# 計算量は O(n^3)/tick と重い。
#
# **代償がある。** 遮蔽を入れると弓の値付けが 1.0129 → 0.7836 へ動く（弓に素の能力を
# 28%足さないと公平にならない）。矢数だけのときは 0.9958 で「人為的な調整がゼロ」
# だったので、そこからは一歩後退している。1.0 からのずれ幅で見ると、遮蔽前の 1.01 に
# 対して遮蔽後は 0.78 で、破綻ではないが小さくもない。
# **陣形の軸（推移幅 0.687 → 1.190）と引き換えである。** 陣形を戦術の軸にしないなら
# 0.0 に戻すのが正しい。判断が要る場所なので、値と代償を並べて残す。
