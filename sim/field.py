...
TYPE_EDGE_COST = {(INF, CAV): -1.3079, (CAV, ARC): 2.6887, (ARC, INF): 4.7798}
...
# 陣形の推移成分（その陣形が「単に強い」ぶん・コスト点）。simulate が
# 兵種相性と同じ形（総コスト30への加算）で**打ち消す**。鍵は前衛の枚数。
# 値付けの経緯と測り直しの則は simulate 内の注記と §7.53。
FORM_SURPLUS = {4: 3.1, 3: -3.0, 2: 0.0}
...
def simulate(a: Army, b: Army, dt: float = 0.25, t_max: float = T_MAX,
    ...
    **同じ編成どうしでも、種を入れれば結果は五分にならない。** それが乱数を
    ...
    # 陣形の推移分の相殺（§7.53）。三すくみ（FORM_BONUS の循環）とは別に、
    # 陣形には「単にどれかが強い」推移成分がある。物理層で 広-標 +1.17 点
    # （§7.39）に加え、ゲーム層ではデッキ経済（前衛4枠を近接で埋める編成の
    # 強さ）が乗る。兵種と同じくコスト点の加算で打ち消す。値はゲーム層の
    # 強化計器（陣形ごとにデッキ4種×乱数・scratchpad formwin2）で合わせた
    # 実測。**プールを変えたら測り直す。**
    fe = FORM_SURPLUS.get(a.form.n_front, 0.0) - FORM_SURPLUS.get(b.form.n_front, 0.0)
    ka = 1.0 + (te - fe) / 30.0
    kb = 1.0 - (te - fe) / 30.0
...
def cmd_formation(args) -> None:
    ...
    （魚鱗対鶴翼はほぼ五分）。いまは推移分を FORM_SURPLUS（コスト点）で
    打ち消し、三すくみは FORM_BONUS の循環が担う（§7.53）。
    ...
    # 推移分の相殺（FORM_SURPLUS）も切る。この計器の仕事は**生の幾何**を
    # 測ることで、相殺を入れたままだと相殺後の残差しか見えなくなる。
    keep_fs = dict(FORM_SURPLUS)
    FORM_SURPLUS.update({k: 0.0 for k in keep_fs})
    try:
