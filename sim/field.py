...
================================================================================
 3. 陣形（ZOC では動かない。効くのは前後の距離）
================================================================================
■ 現状（正しい幾何、コスト点）

       攻＼守          標準     広く浅い   狭く深い
       標準(前衛3)     +0.000   +0.149   +0.217
       広く浅い(前衛4)  -0.149   +0.000   +0.512
       狭く深い(前衛2)  -0.217   -0.512   +0.000
       推移幅 0.365 / 巡回 +0.148

■ **ZOC の影響距離・強さでは変わらない。**（ユーザの案を実測した結果）
   まず、ZOC が「取り付く動き」と「通り過ぎる動き」を同じだけ制動していたため、
   強くすると迂回を高くする前に**正面の接敵が死んでいた**。

       ZOC_STR   0.4 → 歩兵の経路到達 100%
                 0.8 →  69%（残存 0.755）
                 1.6 →  12%（残存 1.000 ＝ 誰も接敵できない）

   そこで `_plan_paths` が既に持つ迂回の度合い `detour` を制動へ掛け、
   **敵を通り過ぎる動きにだけ課す**ようにした。以後どの強さでも接敵は正常。
   しかしそれでも陣形の差は育たない。

       ZOC_STR   0.0    0.4    1.6    3.2    6.4
       標準/広浅 +0.523 +0.508 +0.425 +0.344 +0.420   （単調ですらない）

   **理由**: ZOC は迂回の到着を遅らせるだけで、遅れた騎兵も結局は戦う。総残存
   兵力率で勝敗を決めるかぎり、遅延は陣形固有の差にならない。

■ **効くのは前後の距離だった。** 陣形は (前衛枚数, 正面幅) しか持っておらず、
   **前後の位置関係が変数になっていなかった**（どの陣形でも前衛|y|=30・
   後衛|y|=150 固定）。ZOC が噛む相手が存在していなかった。`Formation.rear_gap`
   を足して測ると、単調で大きい軸になる。

       後方   40m    60m    80m   120m   160m   220m
       優位 +0.935 +0.693 +0.417  0.000 -1.518 -3.253   （基準=120m）

   幅 4.2 コスト点で、陣形3種の 0.365 の **11倍**。ただし**単調**なので
   「浅いほど強い」であって選択にならない。深く置くことに見返り（後衛の保護、
   迂回への耐性）を持たせて初めて軸になる。マスごとのばらつきが ±3.2 と大きい
   点も要注意。
