...
- 兵種の三すくみ: **まだ立っていない**（§2）。値付けは揃った（推移幅 0.005）。
...
================================================================================
 2. 兵種の三すくみ（正しい幾何では**まだ立っていない**）
================================================================================
■ 経緯として押さえておくこと（これは有効）
   - **止めていたのは兵種補正ではなく行動面の係数だった。** §6.1 の
     歩1.00/騎0.90/弓1.12 は旧レーンエンジンの値で、入れたままだと兵種補正を
     0→0.15、FOCUS を 0→3 まで振っても三すくみが 0/100/0 から動かない。
   - **勝率では測れない。** 総コスト +0.1点（30点中0.3%）で勝率が 100% になる。
     決定論の盤面ではわずかでも強い側が全マスで勝つので、勝率は 0/50/100 の
     階段にしかならない。§5.3 の目標帯「55〜80%」はこの計器では踏めない。
     **帯が踏めないのであって調整が下手なのではない。** 以後コスト点で測る。
   - トーナメント表は**推移成分**（単にどれかが強い）と**巡回成分**（本物の
     三すくみ）へ分解できる。**推移成分だけが値付けで消せる。**

■ 正しい幾何で値付けし直した（`field.py price`、rot=6）。収束が遅いので緩和
   係数 1.8 を掛けて2回で収束させた。

       歩 1.0000 / 騎 1.0391 / 弓 1.4407

■ 三すくみ（コスト点、総コスト30に対して）

       兵種補正   歩→騎   騎→弓   弓→歩    巡回c    推移幅
       0.000     -0.243  -0.248  -0.238  -0.2433    0.005
       0.020     +0.075  -0.154  -0.024  -0.0345    0.119
       0.040     +0.390  -0.062  +0.189  +0.1721    0.235
       0.060     +0.703  +0.025  +0.401  +0.3763    0.351
       0.100     +1.321  +0.192  +0.822  +0.7782    0.586

■ **まだ立っていない。理由は2つある。**
   1. 補正 0 の素の巡回が **-0.243**（騎→歩→弓→騎 と逆回り）。3方向が揃って
      いる（推移幅 0.005）ので値付け自体は正しいが、幾何が逆向きの三すくみを
      作っている。補正でこれを打ち消すには 0.035 以上が要る。
   2. **兵種補正が純粋な巡回項でなくなった。** 推移幅が補正に比例して増える
      （0.005 → 0.586）。補正は割合で効くので、射程で長く撃てる弓兵ほど同じ
      %でも得をするため。逆回りを打ち消すのに必要な補正量では、推移の偏りが
      巡回と同じ大きさになってしまう。

   したがって**補正の値をいじるだけでは three-way にならない**。素の巡回を
   正向きに変える構造（接敵抑制の効き方、迂回の価格、射程の減衰など）を先に
   決める必要がある。**次に着手すべきはここ。**
...
def simulate(a: Army, b: Army, dt: float = 0.25, t_max: float = T_MAX,
    ...
    # 陣形相性（表で決める場合）。循環で勝っている側の与ダメージへ掛ける。
    dr = (a.rank - b.rank) % 3
