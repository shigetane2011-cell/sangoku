...
================================================================================
 ...
 - 兵法はゲージ機構が要る。誘発型のうち列指定（殿・鼓舞）とゲージ依存
 ...
 - 兵法・固有特性・状態効果はまだ載っていない（最小モデルのため）。
...
# 戦闘ルール版（§7.135）
# ============================================================================
# 通常攻撃計算・命中率・クリティカル・ゲージ自然増・与ダメ/被弾/討取ゲージ増・
# 的取り・兵法発動条件・固有特性処理をまとめて指す運用上の版番号。武将版
# （generals.csv の「版」列）とは別軸 — こちらは盤面のルールそのものが
# いつ変わったかの記録で、試合記録（players.battles.rule_version）に
# 対戦時点の値をそのまま焼き込む。今回は現行ルールをそのまま 1.0 とするだけ
# で、上げどきの判断はルールを実際に変えるときに改めて行う。
BATTLE_RULE_VERSION = "1.0"
...
# 弓の射程（縁からの距離）。**後衛が接敵前に一方的に撃てない距離にする。**
#
#   90  = REAR_Y - FRONT_Y - CARD_D … 両軍の前衛が噛み合ってから敵前衛へ届く
#   150 = (REAR_Y + FRONT_Y) - CARD_D … 接敵前から敵前衛へ届く（以前はこちら）
#
# 前衛・後衛の兵種を規則にした（後衛は弓兵だけ・§4.1）ことで、150 だと**弓兵が
# 多い陣形が一方的に強くなる**（弓4 が弓2 に +10.7 コスト点。狭く深いが支配的）。
# 原因を切り分けると、矢の量・密着の抑制・後衛の位置はどれも効かず、**射程だけが
# 担っていた**（密着の抑制が効かないのは、後衛の弓兵が密着されないからで、
# それ自体が「後衛が安全すぎる」ことの裏づけである）。
#
# 90 にすると3陣形の幅が 16.39 → 0.98 コスト点になる。弓兵は近接を支援できるが、
...
# 後衛の槍が弓と同じく居残るか（§7.107）。False だと歩兵として前進する。
#
# **True にした。仕様が書いてあるとおりに動かす。** §7.57 は「後衛からは
# 前線越しに突きを入れる」と書いているのに、`_plan_paths` が弓兵しか
# 免除しないので、後衛に置いた槍は 212m 歩いて敵前線まで出ていた。つまり
# 後衛の槍ではなく**4枚目の前衛**だった。
#
# これが分かるまで、つまみを2つ空振りしている。前進できるうちは槍の値打ちが
# 「余分な近接の体」なので、**突きの威力を 0.5→0.15 と3分の1にしても勝率は
# 2ポイントしか動かない**（守りを弓兵並みに落としても5ポイント）。居残らせて
# 初めて値打ちが「後衛から突く火力」になり、威力が効くようになる:
#
#   コストも役割も揃えた対照（王平 槍・均衡5 対 馬謖 弓・均衡5）で槍側の勝率
#     現行（前進する）                        100%
#     居残らせる                               95%
#     居残らせる＋守りを弓兵並み                  92%
#     居残らせる＋守り＋威力0.25                  13%   ← ここで初めて効く
#
# **死んだつまみを掃引して「効かない」と結論しない。** 先に、そのつまみが
# 何に効くはずなのかを確かめる（§13）。
#
# 【§7.112】居残りは**槍の値打ちをほぼ丸ごと消す**。同じ6枚で置き場所だけを
# 変えて測った、後衛へ置く得（中央・コスト点）:
#
#   SPEAR_REAR / SPEAR_GUARD      後衛へ置く得
#     0.5 / False（§7.77 の較正時）    +0.733   ← 値札 0.75 と一致（陽性対照）
#     0.4 / False                     +0.476
#     0.5 / True                      -1.430
#     0.4 / True（いま）               -1.775   ← 後衛へ置くと必ず損
#
# 効いたのは **SPEAR_GUARD のほう（-2.16）** で、威力 0.5→0.4 は -0.26 にすぎない。
#
# 【一度 False へ戻して、盤面が却下した】値打ちが 0 なら語として死んでいる、と
# 考えて居残りを外し値札を 0.48 へ付け直したが、7つの型の総当たりで **⑦魚3弓2槍が
# 87.2% で全型に勝ち越し**、幅が 25.7 → 54.3 ポイントへ開いた。後衛槍の値打ちの
# 出どころは突きの威力ではなく**前へ出られること**で、前衛3・後衛3という枠の
# 予算をすり抜けていた（§7.107 が「威力を 0.5→0.15 にしても勝率が2ポイントしか
# 動かない」と測ったのと同じ話の裏側である）。
#
...
# 敵1体の兵法が、スナップショット兵力に対して**その一撃単独で致死**の
# とき、生ダメージの余り×この割合を最寄りの敵へ通す（第二対象の防御・
# 兵法防御を1回適用・反射なし・連鎖なし）。捨て札（§7.73 ③）が超過分を
# 蒸発させて避雷針になる旨味を消すのが狙い。
TRAMPLE = 0.5            # 余勢の率（§7.76 後記の掃引 0/0.4/0.5/0.6 で採用）
...
#   0.40    47.1%    50.0%    19/37   ← 中央も組数も同時に釣り合う
#   0.35    33.6%    33.3%    11/37
#   0.30    21.6%    16.7%     3/37
#
# 組ごとの幅（0〜96%）はどの値でも残る。札ごとに役割も兵法も違うので当然で、
# つまみは分布全体を平行移動させるだけである。旧値 0.5 は前進していた頃の値。
#
# 【§7.112】上の表は `SPEAR_GUARD = True` の下で取ったが、その居残りは
# §7.112 で False へ戻した。**この 0.40 は据え置いている** — 弱体の狙いぶんは
# 威力側だけで足り、居残りは語ごと殺していたためである。0.40 / 居残り無しの
# 後衛へ置く得は +0.476 で、`design.SPEAR_PRICE` はこれに合わせてある。
# 【§7.254 で試して戻した】0.40 → 0.70 → **0.40**。
#
# 狙いは「後衛へ置いても1コスト点の値打ちが変わらない」ところで、同じ札を前後へ
# 置き替えると 前／後 の比が 均衡 1.51 → **0.99**・耐久 1.34 → **0.94** と、
# 狙いどおり 1.00 に乗った。7つの型の総当たり（balance_suite quick）も
# **全型へ勝ち越す型: なし**のまま通った。**それでも戻した。3辺が割れたため。**
#
#            騎→弓    弓→歩    歩→騎
#   0.40     +5.4     +5.8     +2.8     ← いま
#   0.70     **−7.0**  +9.4    +10.8    ← 騎→弓 が**符号ごと裏返る**
#
# あわせて型の分布も ⑥雁2弓3槍 が 51.9% → **69.8%**（次点 53.6%）と独走した。
#
# **理由**: 後衛の槍は**回り込んだ騎兵に取り付かれる側**にいる。威力を 0.40 → 0.70
# （＝出力 +75%）にすると、そこが**騎兵を止める壁**になり、騎→弓 の辺が消える。
# 三すくみは「騎兵が回り込んで弓を潰せる」ことを土台にしているので、
# **槍の後衛威力は弓の生存を決めるつまみでもある**。
#
# `design.SPEAR_PRICE` は 0.00 のまま動かしていない。動かしたら測り直す決まりに
# 従って測ったが、`skill_price.py --spear` の「後衛へ置く得」は
# **0.40 で −5.19 / 0.70 で −4.66** と依然大きく負で、請求できる返りが無かった
# （＝この変更は値札を払わない純粋な強化だった。だから辺が割れた）。
SPEAR_REAR = 0.40
# 後衛に置いた槍の**守り**（§7.107）。0 で歩兵のまま、1 で弓兵並みまで落ちる。
#
# 槍の強さは火力ではなく**生存**だった。後衛の1枠を弓から槍へ替えるだけで
# 97% 勝つ（陣形も前衛も同じ統制ペアで実測）。帳簿では後衛の被弾が -32%、
# 与ダメが +38%。威力（SPEAR_REAR）を 0.5 → 0.15 と3分の1にしても勝率は
# 2ポイントしか動かず、**つまみとして死んでいた**。
#
# 出どころは兵種の素の守りである。コスト6・均衡で 歩兵は防御56.6・兵力10829、
# 弓兵は防御34.0・兵力16789。後衛は前線越しに撃たれる場所なので、そこへ
# 防御56.6 の歩兵を置くと**後衛が第二の壁**になり、しかも前衛が破れたら
# 普通に近接で戦える。二重取りだった。
#
...
# 【§7.161 で 騎 1.50 → 1.53】§7.157（ダメージの単価 ×3.7）の後、辺が 騎→弓 +11.4／弓→歩 +8.8／
# 歩→騎 +1.5 と歩→騎が消えかけた（§7.160）。type_edges（8組×種2）で 1.53／1.56／1.60 を測り、
# 帯（+5〜+10）に入る最小の動き 1.53 を採る: +7.2／+8.8／+6.4・最強の型 ⑥ 57.1%。
# 1.56 は騎→弓 +2.7 で下へ抜け、1.60 は −3.3。つまみは急なので 0.01 刻みで扱うこと。
ACT_COEF = {INF: 1.0000, CAV: 1.5300, ARC: 0.6032}
# 【§7.124】弓 0.6450 → 0.5142（実効+25%の補償）。時間給の半減（GAUGE_PER_SEC
# 1.6）が「後衛で守られて立つ弓」から兵法を奪い、純弓型がラダーで 39.7→28.6% へ
# 沈んだため。§7.122 の但し書き「採用率で弓が沈む兆候が出たらそのとき再考」の
# 条件が成立した初適用。値は時間給1.6の下で `price` が出した提案そのまま。
# 騎 1.1834 の提案は §7.122 どおり**適用しない**（捕食辺を消しに来る側の読み）。
...
# 【§7.187】1.92 → 1.60。矢数の作り替え（§7.186）と弓の係数 0.61 で 騎→弓 が
# +12.6（目標 +8）まで開いたので、**このマスだけ**で戻した。1.50/1.60/1.70/1.92 の
# 4点で 騎→弓 が +6.5/+8.3/+9.9/+12.6 と単調で、**弓→歩 は4点とも +4.6 で動かない**
# （6マス表にした狙いどおり、辺が分離できている）。
# 【§7.192】2.20 → 2.253。回復の作り替え2回（§7.190/§7.191）で辺が 1.5 → 6.2 まで
# ずれたので、3つのマスを同時に解いて詰め直した（弓 0.59→0.587・このマス 2.20→2.253・
# INF_VS_CAV 0.677→0.632）→ 騎→弓 +8.7・弓→歩 +4.6・歩→騎 +5.3（誤差 **1.4**・型の幅 20.7pt）。
# 【§7.188】1.60 → 2.20。値札を引き直した（安売りだった攻撃バフ・混乱の持ち手が弓に
# 偏っていたので、是正がそのまま弓の弱体になった）ぶんを、**3つのマスを同時に解いて**
# 戻した — 弓の係数 0.61→0.59（弓→歩）・このマス 1.60→2.20（騎→弓）・
# INF_VS_CAV 0.49→0.677（歩→騎）。3辺の誤差の和 20.0 → **1.5**。
# 【§7.193】2.253 → 1.983。兵法防御の単価を測り直した（§7.193）ぶんを持ち手3枚の
# 身体へ返したら、そのうち2枚（陸抗・郝昭＝**歩兵の耐久**）が雁行の前衛の常連で、
# 弓型 ②雁2弓4 が 44.1→50.1% へ跳ね、辺が 1.4 → 6.2 までずれた。3つのマスを同時に
# 解いて詰め直す（弓 0.587→0.6032・このマス 2.253→1.983・INF_VS_CAV 0.632→0.5807）。
# 【§7.196】1.983 → 1.555。**目標そのものが変わった**（テストプレイの決定
# 「騎→弓 +8.7 を +5 にしてみよう」）。§7.145 以来の的 騎→弓 +8 は、探索が弓を
# 6枚しか使わない（殿堂12組が全部6枚・在野は 8.2枚）原因を辿った先の裁定で、
# **三すくみを +5／+5／+5 の均等**へ変えた。弓の係数は据え置き（動かすと名簿の
# 弓兵50枚が全部動くので、必要が無ければ触らない）で、このマスと
# INF_VS_CAV 0.5807→0.5622 の2本だけで解いた。
# 【§7.197】1.555 → 2.124。諸葛亮〔臥龍〕を10コストの瞬発（敵前衛への一撃）へ作り替えたら
# 歩→騎 が +5.2 → **+1.7** まで落ちた（前衛をまとめて焼く弓が入り、厚い前衛の歩型が損をする）。
# 的（+5/+5/+5）は変えずに2本で解き直す（このマス 1.555→2.124・INF_VS_CAV 0.5622→0.6629）。
# **札1枚でここまで動く**（落とし穴46）。
# 【§7.229】2.124 → 1.5368。§7.224（知略の対象倍率を正面と同額へ）で騎兵5枚が弱くなり、
# **歩→騎 が +6.4 → +8.4** へ開いた（§7.227）。的（+5/+5/+5）は変えずに、
# **いまの動作点で感度を実測してから**2本で解き直した（1単位あたり
# INF_VS_CAV: 騎→弓 −89.0／歩→騎 +55.6 ・ このマス: 騎→弓 +15.3／歩→騎 −3.7。
# **handoff の −61／+48 は昔の動作点の値なので使わない**）。
# 解いた点で1回測り、残差でもう1歩詰めた着地。**弓→歩 はこの2本では動かない**
# （1単位あたり +0.5／+0.1）ので +5.7 のまま＝誤差の下限 0.7。
CAV_VS_ARC = 1.5368
# 【§7.124】0.30 → 0.60。弓の値札補償（ACT_COEF 0.5142＝兵力+25%）で騎>弓の
# 辺が 24→17/24 へ緩んだぶんの取り戻し。0.45/0.60 の2点で 19/21 と単調、
# 他の指標は全て不動（接敵が門なので城塞にもラダー幅にも波及しない）。
...
INF_VS_CAV = 0.5572    # 歩→騎（槍衾で馬を止める。後衛の槍とは別に、歩兵一般の値）
                       # 【§7.229】0.6629 → 0.5572。CAV_VS_ARC と同時に解いた（上の注記）。
                       # 【§7.188】0.49 → 0.677。**このマスは効きが強い**: 0.49→1.00 で
                       # 歩→騎 が −1.6 → +22.9、同時に 騎→弓 が +6.1 → −25.1（1単位あたり
                       # 歩→騎 +48・騎→弓 −61）。単独で振らず3つ同時に解くこと。
INF_VS_ARC = 0.0       # 歩→弓
CAV_VS_INF = 0.0       # 騎→歩
ARC_VS_CAV = 0.0       # 弓→騎（いまは馬上回避 CAV_COVER が担っている向き）

# 【なぜ他の兵種補正をここへ集めないのか — 2026-09-10・調べて閉じた課題】
# 「接敵まわりの兵種補正（馬上回避・後衛の槍・弓の接近戦の防御）を全部この表へ
# 統合する」という課題を立てていたが、**書かれたとおりには実行できない**と分かった。
# 次に読む人が同じことを試して壊さないよう、理由を残す。
#
# ① **槍は兵種ではない。** `SPEAR_REAR`（後衛の槍は威力4割）は Unit の組み立てで
#    `self.atk *= SPEAR_REAR` と**攻撃力そのもの**を下げる処理で、**的の兵種を見て
#    いない**。この表は「攻め手の兵種 × 的の兵種」なので形が合わない。
#    `SPEAR_REAR_DFN`・`SPEAR_MELEE_BACK` も同じく武将1人の状態。
#    `SPEAR_VS_CAV` だけは「槍→騎」で表の形をしているが、**行の鍵が兵種ではなく
#    「槍を持っているか」というフラグ**なので、鍵の空間を広げる作り替えが要る。
# ② **馬上回避は兵法にも効くが、この表は効かない。** `type_atk` を呼ぶのは
#    通常攻撃の2か所だけ。`CAV_COVER` は `CAV_COVER_SKILL` で**兵法の直撃にも**
#    掛かる。ここへ移すと兵法に対する馬上回避が**黙って消える**。
# ③ **距離の消え方が違う。** 馬上回避は射手の縁から `CAV_COVER_SOFT`（40m）の幅で
#    消えるが、`TYPE_ATK_NEAR` は `smooth_gate(d, 0, SUPPRESS_R)`。曲線が違うので
#    値をそのまま移しても同じ数字にならない＝「1点も動かさない作り替え」が成立しない。
# ④ **弓の接近戦の防御は防御力であってダメージ倍率ではない。**
#    `ARC_MELEE_DEF_LOSS` は的の `dfn` を下げる。`(100+防御)` という別の曲線を通るので、
#    `hit` へ掛ける倍率とは同じ量ではない。
#
# 実質移せるのは値が 0 の4つだけで、それも形が合っていない。やるなら**表の鍵を
# 兵種の外（槍・距離）まで広げる作り替え**として仕切り直すこと。いま困っていることは無い。
...
# 2026-08 再較正（§7.55・相性の圧縮）: 目標を 1.0 → 0.6 コスト点へ下げた。
# テストプレイの指摘「相性が6:4は大きすぎる。5.5:4.5くらいに」。旧表の実測は
# 0.95〜1.21 と狙いの 1.0 も超過していた。calibrate --target 0.6 --skills で
...
FOCUS = 1.75             # 射撃の集中度。§5.2「残兵力の少ない方を狙う」を連続化した
...
# 【§7.237】騎兵 42.3 → 46.0（テストプレイの決定「騎兵46」）。
# 汜水関の上限を20にしたら騎兵が**両方の辺で負けた**（騎→弓 -1.7・歩→騎 +11.0）。
# 原因は身体の配分: 兵力は `コスト曲線 ÷ ACT_COEF` なので騎兵は 1/1.53 に割られ、
# 防御力はここで兵種ごとに固定されていてコストでも役割でも動かない。
# 規則で前衛に置けるのは歩兵と騎兵だけなので、**騎兵は壁の適性がないまま壁をやらされる**。
# 掃引（type_edges 8組×種2・4,608局）: 42.3→44→46 で 騎→弓 -1.7→+1.1→+5.4、
# 歩→騎 +11.0→+9.0→+6.1、**弓→歩 は +3.0 で完全に不動**（騎兵が絡まない辺なので当然）。
# つまみが騎兵の2辺だけを動かすので、他をこじらせずに詰められる。
# **これは配分替えではなく買い増し**である: 防御力は総合値の式に入っているのに
# 兵力・攻撃力が下がって相殺されないので、騎兵の総合値が +1.29% 上がる（他兵種 0.00%）。
# 型の最大が 55.7% → 61.9% となり **§7.145 以来の的（60%）を割る** — 要監視。
DEF_BY_TYPE = {INF: 56.6, CAV: 46.0, ARC: 34.0}
...
# 兵法の係数元。兵法の性質で 武力 と 知力 の配合を変える。
#   係数 = 武力 × (1 - v) + 知力 × v
# 単体の斬撃は武兵法(v=0.15)、継続・妨害は計略(v=0.85)、範囲打撃はどちらでもある
# (v=0.50)。範囲を一律「知力」にすると呂布の無双乱舞が計略扱いになって不自然だった。
# **武兵法は 0.15 ではなく 0.00。** 攻撃力の知力比（INT_WEIGHT）は歩騎で0.10なので、
# 武兵法を0.15にすると「兵法のほうが攻撃力より知力を重く見る」ことになり、武力へ寄せた
# 札の兵法の係数が下がる。実測で勇将（武力104.7）の武兵法の係数は 97.6 で、中庸の100を
# 下回っていた。**どの兵法種でも武力寄りが損をする＝勇将が支配される選択肢**になる。
...
# 序盤の差が最後まで雪だるま式に効く（§6.1 の増幅）。兵法を武力・知力ベースに
# すると分母に兵力を持たないので、**劣勢側ほど兵法の比重が上がる**。構造的な
...
# 1回だけ継ぎ足し、抜けた隊には前衛の遮蔽を掛けない）、文鴦で測った:
#
#   案                         勝率     残存差    強弓   槍陣
#   一撃だけ（敵前衛 威力400%）  98.3%   ±0.0000   100%   100%
#   突破（走って抜ける）         94.6%   −0.0205    85%    75%
#   突破（その刻に抜ける）       89.2%   **−0.0354**  **25%**  75%
#   突破＋自分バフ             92.5%   −0.0283    45%    85%
#
# **抜けるほど悪くなる。** 与ダメも 9,351 → 7,755 → 6,633 と落ちる。理由は
# 構造的で、盤面は**列を保つことに報酬を与えるよう較正されている** ——
# 前衛の遮蔽（§7.151）・ZOC（§7.122）・射撃の集中（FOCUS）の3つが揃って
# 「単騎で前へ出た者は集中砲火で溶ける」を作っている。列を自分から離れる札は
# この較正と正面からぶつかるので、**後衛の弓に触れられる得より、味方の援護を
# 失う損のほうが大きい**。「その刻に抜ける」が最悪（強弓 25%）なのが決定的で、
# 移動時間が原因ではなく**孤立そのもの**が原因だと分かる。
#
# 入れるなら盤面の側（遮蔽・ZOC・集中）を作り直す話になるので、器1つでは足りない。
# 実装は戻してある（`Unit.set_path` の「戦闘中は経路を変えない」も守れる）。
...
# 兵種ごとの「兵力 ↔ 一撃の重さ」の振り分け（§7.150）。**既定 1.0 は挙動不変。**
#
# コスト曲線の増分は設計式の構造上ぜんぶ兵力へ乗る（design.derive: men だけが s を
# 受け取り、攻撃力は役割だけで決まる）。兵種係数 ACT_COEF もそこへ乗るので、弓は
# 同コストで歩兵の約2倍の**兵力**を持つ（実測 1.85〜2.21倍・攻撃力は 0.94〜1.41倍）。
# 絵として「弓が一番数が多い」のは変であり（テストプレイの指摘・歩兵が一番安い）、
# 盤面でも §7.150 の裁定の原因になっている — 罰（接敵抑制・矢切れ）は出力にしか
# 掛からないので、兵力で受け取った報酬には罰が届かない。
#
# 総合値 = 兵力 × √(攻撃力/間隔 × (100+防御)/100) なので、**兵力 ×x・攻撃力 ÷x²**
# なら1枚の総合値は保たれる（帳簿は動かない）。x < 1 で「人数は減るが一射が重い」
# 形になり、盤面では 与ダメ(兵力×攻撃力) が 1/x 倍・耐久が x 倍になる。
# 値はテストプレイの決定を待つ（掃引のためだけに置く）。
TYPE_MEN_SPLIT = {INF: 1.0, CAV: 1.0, ARC: 0.54}
...
# 【§7.74 で 0.20 → 0.60 へ・兵法にも掛ける】「密着された弓は撃てない」を
# 実効にする。0.20 では取り付かれた弓の出力が74〜87%残り、改設計弓の武力
# （コスト比で騎兵の2〜4倍）が白兵でも勝ってしまい、カウンターが立たない
# （§7.73 の解剖）。0.60・兵法含で 束ね＞普通・騎兵＞束ね(77%)・普通＞騎兵 の
# 三すくみが閉じ、槍後衛が騎兵への迎撃(82%)になる（§7.74-75）。
# 値はプールの値付けと対 — 動かしたら §7.77 の再較正を回し直すこと。
SUPPRESS_MAX = 0.85     # 密着したときに弓が失う出力の割合
...
# 同一実行内で取り直すこと。
AMMO_MODE = "shots"       # "time" | "attrition" | "volley" | "shots"（§7.187 で採用）
...
AMMO_SHOTS = 9.0        # "shots" のときの持ち矢（満身で撃ち続けられる秒数・§7.187 で採用）
...
#
# これで知力が**守りにも効く**ようになる。いまの知力は兵法の係数にしか効かず、
...
#
# 掃引（陣形×前衛の兵種×後衛の中身 7種の総当たり。tools/ladder_top.py combo）:
#
...
#
# **幅が最小になる 0.60 を採る。** 0.50 だと騎兵が戻りすぎて今度は ① が
# 26.5% へ潰れ、0.70 だと騎兵が戻りきらない。端点ではなく本物の谷である。
...
# （零点・dt不変は従来と同一）。CAV_COVER_SKILL は兵法の直撃にも掛けるか。
CAV_COVER = 0.45        # 接敵前の騎兵が射撃被害を免れる割合（0 で無効）
# 【§7.144】0.0 → 0.15 → 0.30（テストプレイ「弓に有利な兵種なのに弓に狩られる側
...
# 量は測って決めた: +4% 39功／+7% 65功／+15% 149功（小の帯なので +4%）。
# **矢を避ける効果は思ったより効く** — 白馬（生来・+0.25）が高く付くのと同じ理由。
TREASURE_KEIKI_COVER = 0.04
...
COVER_NAME = "馬前"     # 実況・詳録に出る名前（traits.csv の名前と揃える。テストプレイの決定）
...
# 無くなったので、§4.6 の「内訳で表現する」方針は兵法の設計そのもの
# （知力ベースなど）で使えばよい。
...
# 陣形どうしの**残差の表**（コスト点・反対称）。(a, b) の値は「a が b に対して
# 実測でこれだけ強い」— simulate が兵種相性と同じ形（総コスト30への加算）で
# 打ち消す。推移成分（どれかが単に強い）も循環の過剰も、3つの反対称成分で
# 全部ここに畳める（1形1値の旧 FORM_SURPLUS では循環を削れなかった）。
# 目標: 相殺後のゲーム層の各辺 57%±2（§7.55。相性を6:4→5.5:4.5へ圧縮）。
# 鍵は前衛の枚数。**プールや兵種相性の狙いを変えたら測り直す**（広い計器）。
# 2026-08-25 引き直し（§7.104）。雁行の深さを実装したので、**雁行の2辺は
# 相殺が要らなくなった**（0.0）。残るのは 魚鱗-鶴翼 の辺だけ。
#
#   深さ 1.12x での実測: 魚→鶴 56.5% / 鶴→雁 59.3% / 雁→魚 54.9%（狙い 57%）
#
# 深さを入れずに表だけで揃えることもできる（実測済み）が、
# {(3,4): -4.2, (4,2): +11.6, (2,3): -9.8} と大きくなり、+11.6 は 30点の軍の
# 39% ＝ 鶴翼対雁行の勝敗を表が4割決めることになる。しかも相殺は与ダメージを
# 削る形で効くので、**前衛が全滅したのに勝つ雁行**が量産されて実況が嘘になる。
FORM_PAIR = {(3, 4): -4.2, (4, 2): 0.0, (2, 3): 0.0}


def form_pair(na: int, nb: int) -> float:
    """陣形ペアの残差（コスト点・反対称）。同じ陣形どうしは 0。"""
    if (na, nb) in FORM_PAIR:
        return FORM_PAIR[(na, nb)]
    if (nb, na) in FORM_PAIR:
        return -FORM_PAIR[(nb, na)]
    return 0.0
...
TRAIT_TRIGGER = 0.30
# 持重（foe_skill）の助走（§7.129・**実験中のつまみ**）。敵の攻め兵法が通算で
# これを超えてから反応する。0 は「1発目から反応」＝設計書のままの挙動。
# 密度対策として意味があるのは「敵が兵法を連ねてきたときだけ働く」形なので、
# 助走を置くと低密度（兵法が数発しか飛ばない編成）へは効かなくなる。
TRAIT_FOE_SKILL_WARMUP = 0
# 固有特性。**兵法とまったく同じ器で持つ。** CSV の効果文が同じ文法
...
# 陣頭: 前衛に置いたときの兵力の増分。
# **12% は受動効果として重すぎた。** 値段が 1.72コスト点 で、どの兵法（最大
...
# 矢衾（vs_inf・§7.237）。**歩兵にだけ効く上乗せ。**
# 対勢力（VS_FACTION）と同じ「常在で相手を選んで効く」家族だが、器は
# §7.202 の `edge_deal`（この隊だけ相性をずらす）を借りる — 6マスの表と
# 同じ口を通るので、損害の入口に新しい分岐が増えない。
# 狙いは 弓→歩 の辺だけを動かすこと。騎兵との辺（いま壊れている側）には
# 触れないので、対勢力より**副作用が小さい**。
VS_INF = 0.10           # 矢衾: 歩兵へのダメージ増（密集歩兵は矢の的・§7.123）
...
#             守りが効かない。輜重の逸話にも合う）
TREASURE_SEKITOBA_MEN = 0.02   # 赤兎馬: 兵力+2%（武力+5と速度寄せ+0.3は play.py 側）
TREASURE_MOTOKU_SCUT = 0.30    # 孟徳新書: 兵法防御+30%（敵の手を書物で見抜く）
TREASURE_RENDO_ATK = 0.08      # 諸葛連弩: 通常攻撃+8%（弓兵限定はセット時検証）
# 矢はこの範囲でほぼ比例した（+10% 43功／+25% 113功／+60% 269功）。小の帯なので +10%。
TREASURE_MOKGYU_AMMO = 0.10    # 木牛流馬: 持ち矢+10%（弓兵にだけ意味がある）
...
TREASURE_FACTION = {
    # 【§7.203】値段が予算を突き抜けた2枚を弱めた（テストプレイの裁定「高い札は
    # 効果を弱める」）。宝物の予算は 汜水関60／官渡90／赤壁120功 なので、
    # 427功・129功では**どの戦場でも装備できない**札になっていた。
    #   玉璽  気勢 +8% → +2%（427 → 105功。+3% では 161 で赤壁に収まらない）
    #   蜀錦  兵力 +2% → +1.7%（129 → 105功）
    # 効き方はほぼ比例していた（玉璽は量を1/4にしたら値段も約1/4）。
    "t_gyokuji": ("群雄", "rate", 0.02),   # 伝国玉璽: 全軍の気勢+2%
    "t_iten": ("魏", "atk", 0.03),          # 倚天剣: 全軍の攻撃+3%
    "t_sonshi": ("呉", "def", 0.03),        # 孫子十三篇: 全軍の守り+3%
    "t_shokkin": ("蜀", "men", 0.017),     # 蜀錦: 全軍の兵力+1.7%
}
...
# 兵法のゲージ（§7.2）。最大値100を基準に、消費ゲージ%が発動の閾値になる。
# **測定では既定オフ。** 統制した合成カードにはゲージを持たせない。
# **兵法は既定でオン。** 兵法を切った盤面は別のゲームで、そこで較正した係数を
# 兵法オンの盤面へ持ち込むと 弓兵→歩兵 が負のまま残る（実測 -0.83）。
# ACT_COEF と TYPE_EDGE_COST はどちらも兵法オンで測り直した値である。
SKILLS_ON = True
...
# ゲージの自然増加（毎秒）。**1.45 では消費175以上の兵法が一度も撃てなかった**
# （90秒の供給が130しかない）。「弱いけど何度も出る兵法」と「本当の大技」を分けるには、
# 供給が消費の幅より広くなければならない。3.20 なら90秒で約300が入り、
# 消費50が6回・100が3回・200が1回（62秒）・250が1回（78秒）になる。
#
# 供給を上げたぶん兵法が出る回数が増えるので、**SKILL_SCALE を下げて予算を戻す**
# （消費100・威力200% の兵法を持つ価値が +0.0670 のまま変わらない点を探した）。
# 変わるのは兵法1発の重みと出る回数の配分であって、兵法の総価値ではない。
#
# 【§7.124】3.20 → 1.60 へ半減。城塞（壁＋後衛主砲）の兵法回転が「安全に立って
# 貯める」時間給に依存していたため。時間給だけの掃引で官渡城塞 91→70%・
# 集中ギャップ +46→+25pt、三すくみと狙いの辺（騎>雁行・弓>鶴翼）は全部維持。
# 被弾給への振り替え（15→30）は二次的で効かなかった — 効くのは時間給の深さ。
# 90秒の時間給が 288→144 になるので、消費175以上の大技は初期ゲージか戦闘収入
# （与ダメ給・被弾給）が無いと届かない。**安全圏の札ほど兵法が細るのは狙い。**
GAUGE_PER_SEC = 2.80
GAUGE_PER_DEAL = 20.0   # 与ダメージ ÷ 対象の最大兵力 × これ
GAUGE_PER_TAKE = 15.0   # 被ダメージ ÷ 自分の最大兵力 × これ
GAUGE_ON_ROUT = 10.0    # 敵を撤退させたとき — **未配線・現在未使用**（§7.127後記）。
                        # 配線しない判断: 固定10は手数型の回転へ直に効き、低コスト
                        # 速攻・捨て札が相手のゲージ源になる。試すのは Phase B・余勢
                        # の後の別検証で、そのときは固定値でなく「撤退武将のコスト
                        # 比例・生存味方へ均等配分」を候補にする。

# 節制（陸抗・§7.130）。陸抗が自身の兵法を初めて放った後、
...
class Card:
    ...
    # 兵法のゲージ（§7.2）。**3つセットで意味を持つ。**
    #   消費が閾値なので、消費50は約25秒で撃てて消費175は約87秒かかる。
    #   これで「弱いけど何度も出る兵法」と「本当の大技」が分かれる。
    ...
    stat_cost: float = 0.0
    # 決めゼリフ（generals.csv「台詞」）。兵法の初回発動時に実況へ出す。**飾りで
    ...
    # （atk まで掛けると二乗で効く）。兵法は兵力に比例しない設計（§7.5）の
    # ため掛からない — 周回が深いほど兵法より地力の比重が上がる。既定 1.0 で
...
# 掃引（相殺を切り、形の配分を3陣形とも同じ範囲で振って 1377局ずつ。
# tools/ladder_top.py depth）:
#
#   深さ    魚→鶴    鶴→雁    雁→魚    後衛の伸び
#   1.00x   35.8%   95.7%   20.1%      0m   ← 深さ未実装のころ
#   1.10x   35.8%   71.6%   45.1%     12m
#   1.15x   35.8%   35.8%   74.1%     18m
#   1.35x   35.8%    0.9%   99.1%     42m
#
# **雁行の2辺が同じ深さで同時に狙いを通る**（57% の内挿が 1.120 と 1.121）。
# 深さは 魚→鶴 の辺を動かさないので、そこだけ FORM_PAIR で詰める。
# 1辺あたり 459局＝標準誤差 2.3% なので、これ以上の追い込みは雑音を追う。
...
class Unit:
    ...
    def __init__(self, side: int, card: Card, form: Formation,
        ...
        # **design.derive と同じ量をここでも掛ける** — 盤面は stat_cost から兵力を
        # 引き直すので、片方だけに入れると設計式と盤面がずれる。
        sp = TYPE_MEN_SPLIT.get(card.typ, 1.0)
        self.men0 = CARD_MEN * (s ** SPLIT_EXP) * rm * sp \
        ...
        scale = (BASE_ATK * (s ** (1.0 - SPLIT_EXP))
                 / (rm ** ROLE_ATK_EXP) * ATK_BY_TYPE[card.typ] / (sp * sp))
        ...
        # **割り算はここだけ。** 兵法は武力・知力をそのまま使うので、兵法の威力は
        # コストに比例する（消費ゲージだけが兵法の格を表していた状態を直す）。
        ...
        if TRAITS_ON:
            ...
            # 矢衾（§7.237）。固有特性なので兵種は見ない（持ち手が1枚に決まる）。
            if "vs_inf" in self.traits or "vs_inf" in self.hidden_traits:
                self.edge_deal[INF] = self.edge_deal.get(INF, 0.0) + VS_INF
        ...
        if card.spear and not is_front:
            ...
            self.atk *= SPEAR_REAR
            # 身を晒すぶん守りが落ちる（§7.107）。dfn は既に決まっているので
            # ここで弓兵側へ寄せる。**前衛の槍はこの枝へ来ない。**
            self.dfn += (DEF_BY_TYPE[ARC] - DEF_BY_TYPE[INF]) * SPEAR_REAR_DFN
        ...
        # **総量ではなく毎秒で持つ。** 総量を1発で入れると、14秒かけて削る兵法と
        # 同じ量を初手で入れる兵法の区別が消える。前者は相手が先に潰走すれば
        # 取りこぼすし、回復は相手の火力と競争になる。そこが兵法の性格である。
        ...
        self.refl_back = 0.0    # 兵法反射で撃ち手へ返した被害
        self.cut_saved = 0.0    # 兵法防御・通常攻撃防御で減らした被害
        ...
        self.fires = 0      # 兵法の発動回数
        # 構えの帳簿（§7.126・表示専用）
        self.null_blocked = 0    # この隊の構えが打ち消した敵兵法の数
        self.null_names = []     # 打ち消した兵法名
        self.scut_saved = 0.0    # 兵法防御で減らした被害（cut_saved の兵法ぶんだけ）
        self.guard_casts = 0     # 構えの兵法を張った回数（撃ち手側）
        self.guard_idle = 0      # 窓の中で一度も仕事をしなかった構えの数
        self.guard_watch = []    # 判定待ちの構えの窓 (終了時刻, 対象, 基準値)
        self.fire_times = []     # 兵法の発動時刻（§7.127・表示専用）
...
def _fire_traits(ua, ub, t, retired, ev, seen, fired_skill=None,
                 offense=None) -> None:
    """誘発型の固有特性を発火させる（§6.6）。中身は兵法と同じ器で処理する。

    閾値をまたぐ判定なので**分岐**であり、時間刻みに弱い。移動の規則と違って
    連続化できない（「撤退した」は本質的に離散）ので、dt を振って結論の符号が
    変わらないかを別途確かめること。

    fired_skill には、このティックで兵法を撃った札の集合を渡す（ally_skill 用）。
    offense には、**攻め兵法が実際に解決した**札の集合を渡す（foe_skill 用・
    §7.129。打ち消された兵法は入っていない）。
    """
...
@dataclass(frozen=True)
class Skill:
    """兵法1つぶんの中身。CSV の効果文から作る（`_parse_skill`）。
...
def _skill_mods(effect: str) -> Tuple[Tuple[str, float, float], ...]:
    ...
    +30% は敵の速度を上げる兵法ではなく、突っ込む自分の速度である。
    """
    out = []
...
def _skill_targets(target: str, u, foe, own, dead_ok: bool = False):
    ...
    3枚だった。位置と無関係なので、前衛を守る兵法が後衛に飛ぶことがあった）。
    """
...
def _skill_line(u: Unit, name: str, tstr: str, tgts, kind: str,
    ...
    """兵法・固有特性の実況行。**数値ではなく出来事として書く**（§9.3）。
...
def _men_now(f: Unit) -> float:
    """**解決後の兵力**を返す（§7.129）。兵法のフェーズ中は遅延窓の増減を織り込む。

    特性は兵法の後・窓を流す前に撃つので、`f.men` だけを見ると「このティックで
    討ち取られた隊」がまだ生きて見える。持重（敵の攻め兵法への回復）がそれを
...
def _is_offense(sk: "Skill", tstr: str) -> bool:
    """攻め兵法（攻撃・妨害）か。強化・回復・構えは含まない（§7.129）。

    持重の発動条件（foe_skill）が見る。味方向けの兵法は敵に触れないので除く。
    """
...
def _is_guard(sk) -> bool:
    """構えの兵法（兵法防御・反射・打消し）か。§7.126 の先行段で立てる対象。

    威力も回復も持たない純粋な備えだけ。攻めと構えを兼ねる兵法を先行段に
...
def _fire_skills(own, foe, t: float, ev, seen, guard=None,
    ...
    guard（§7.126）: True なら構えの兵法だけ、False なら攻めの兵法だけを撃つ。
...
def _recalc_mods(u: Unit) -> None:
    ...
    §6.5 の重複規則。**素朴に足すと壊れる。** 6枚が同じ兵法を3回ずつ撃つと
...
#
#  兵法のゆらぎ（jitter）… 兵法の閾値を発動ごとに ±SKILL_JITTER 振る。**1戦の
...
SKILL_JITTER = 0.00     # 兵法の閾値のゆらぎ（消費ゲージに対する割合）
...
def _cover_map(units: List["Unit"]) -> List["int | None"]:
    ...
    if not TRAITS_ON or COVER_SHARE <= 0.0:
        return out
    guards = [(k, c) for k, c in enumerate(units)
              if c.men > 0.0 and c.is_front and "cover" in c.traits]
    if not guards:
        return out
    for j, f in enumerate(units):
        ...
        for k, c in guards:
            if k == j:
                continue
            d = abs(c.x - f.x)
            if d <= COVER_ADJ and (best is None or d < best[0]):
                best = (d, k)
        out[j] = best[1] if best else None
    return out
...
def simulate(a: Army, b: Army, dt: float = 0.25, t_max: float = T_MAX,
    ...
    **同じ編成どうしでも、種を入れれば結果は五分にならない。** それが乱数を
    ...
    # 陣形の残差の相殺（§7.53・§7.55）。物理の幾何（広4が単に強い +1.17）と
    # デッキ経済（前衛4枠の値段・弓枠の兵法価値）が作る「陣形どうしの実測の
    # 偏り」を、兵種と同じくコスト点の加算で打ち消し、狙いの辺（57%±2）へ
    # 寄せる。値はゲーム層の広い計器（陣形ごとデッキ4種×乱数）の実測。
    fe = form_pair(a.form.n_front, b.form.n_front)
    ka = 1.0 + (te - fe) / 30.0
    kb = 1.0 - (te - fe) / 30.0
    ...
    for _ in range(steps):
        ...
        # **兵法を先に、固有特性を後に。** 特性の ally_skill 条件は「味方が
        # 兵法を撃った」を見るので、順序を逆にすると1ティック遅れる。
        fired = set()
        ...
        if SKILLS_ON:
            ...
            for x, taken in ((u, d) for u, d in zip(ub, db)):
                ...
                if x.men0 > 0:
                    x.gauge += taken / x.men0 * GAUGE_PER_TAKE
            # §7.126 兵法フェーズの二段化。まず**両軍の**構え（兵法防御・反射・
            # 打消し）を立てて反映し、それから両軍の攻め兵法を同時解決する。
...
# **打撃と計略を別の種類にする。** 同じ「兵法」に混ぜると、行の取り合いで
...
# 節目にする。cap/prio は兵法・計略と同じ重みを壊滅に、苦戦は旧「壊滅」の
# 枠をそのまま引き継ぐ。
# **「兵法」の枠だけ意味が違う**（§7.243）: 必ず出す行（撃った隊の初回・構えの
# 相方・決め手・打消し）はここに数えず、この枠は**2発目以降を何本添えるか**を
# 決める。3 のままだと確保ぶんが枠を先に埋めて大きい2発目が出なかったので 6 へ
# （実測: 2発目以降が出る率 58% → 97%・実況の行数は 26.3 → 26.8 でほぼ変わらない
# ＝他の種類を押し出していない。8 にすると 100% だが、1隊が延々と撃つ長期戦で
# 同じ兵法が並ぶので上限は残す）。
LINE_CAPS = {"布陣": 1, "予告": 2, "結果": 2, "接敵": 1, "抑制": 1,
             "兵法": 6, "計略": 3, "誘発": 2, "突撃": 1, "苦戦": 4,
...
LINE_ORDER = {"兵法": 1, "計略": 1, "誘発": 1, "同士討ち": 2, "結果": 2,
              ...
              "苦戦": 3, "壊滅": 3, "戦況": 4, "決着": 9}
LINE_BUDGET = 20
# 同じ隊の「2発目以降」を何本まで実況へ添えるか（§7.243）。枠（LINE_CAPS の
# 「兵法」）だけで絞ると、大技の2発目が少ない戦いでは小さい兵法の3発目・4発目が
# 枠を埋めて、同じ文が並ぶ（実測で同一の行が4回）。隊ごとに1本までとする。
LINE_REPEAT_UNIT = 1
...
#
#   開戦 8:00 ／ 中央値 118ティック × 1.70 = 201分 → 11:21
#   上位10%（195T）は 13:31、最長（308T）で 16:43
#
# 【§7.151 で 1.35 → 1.70】採用で決着が速くなり（中央値 264→118ティック・
# 時間切れ 18.9%→0%）、1.35 のままだと昼前決着が 86% になって時刻帯の行が
# ほぼ死んでいた。1.70 なら 昼前66% / 12-14時27% / 14-16時7% と、下の帯を
# 較正したときの分布（昼前64% / 17% / 6%）へほぼ戻る。**盤面は触っていない
# （T_MAX も据え置き）** — 表示の尺だけを合わせ直した。
# なお決着が上限（400T）へ届かなくなったので、上限＝日没という対応は使えない。
# 日没の語は「最長の戦い（16:43）」の側に載る。
#
...
MIN_PER_TICK = 1.70     # 盤面1ティックを戦場の何分として読ませるか（表示のみ）

# 時刻帯の行（§9.3）。**`narrate()` の中だけで作る。`simulate()` は時刻帯を知らない。**
# 盤面へ入れないので、表示専用であることが構造で保証される（勝敗にも測定にも
# 触れようがない）。実カード144対戦で測った決着時刻の分布に合わせてある:
#
#   （較正時・実カード144対戦）中央値 11:10 ／ 08-10時 5% ／ 10-12時 64%
#   ／ 12-14時 17% ／ 14-16時 6% ／ 16-17時 1% ／ 上限（日没）到達 7%
#   （§7.151 後・7型1008局）中央値 11:21 ／ 〜12時 66% ／ 12-14時 27%
#   ／ 14-16時 7% ／ 16時〜 0.3%。**16時の行は最長級の戦いだけに出る。**
#
...
class Event:
    ...
    # 兵法は1戦に18回ほど飛ぶので、先に起きたものから採ると小物ばかり並ぶ。
    mag: float = 0.0
    # 台詞が紐づく本体行の id()。**本体が枠から落ちたら台詞も出さない**（§9.4。
    # 兵法の行が消えて声だけ残る「孤児の台詞」が実測で出た）。0 なら独立行。
    ref: int = 0
...
def _who(u: Unit) -> str:
    ...
    （実測: 両軍の満寵が同じ刻に兵法を撃ち、2行が区別不能だった）。その名に
...
def _log_tick(ev, seen, t, ua, ub, gap) -> None:
    ...
    # 兵数の推移はこの行が担う（兵法や壊滅の行は「なぜ減ったか」を担う）。
...
def _arrange(ev: List[Event], casts: list, score: float):
    ...
    # **兵法の枠は「2発目以降を何本添えるか」にする**（§7.243）。§7.242 で
    # 「撃った隊の初回」「構えの相方」を必ず出すようにしたら、確保した行が
    # 枠（3本）を先に埋め切ってしまい、**大きい2発目が一本も出なくなった**
    # （テストプレイの決定「長くなってもいいから大技の2発目も出して」）。
    # 確保ぶんを枠に数えず、枠は下の取捨（初回より後・大きい順）に丸ごと渡す。
    # ここだけの扱いで、他の種類（壊滅・苦戦・戦況…）は今までどおり数える。
    used["兵法"] = 0
    n_opt = 0
    rep_by_unit: Dict[tuple, int] = {}
    for e in sorted(ev, key=rank):
        if id(e) in kept_ids or e.kind == "台詞":
            continue
        if used.get(e.kind, 0) >= LINE_CAPS.get(e.kind, 1):
            continue
        if e.kind == "兵法" and e.cast:
            # **同じ隊の2発目以降は LINE_REPEAT_UNIT 本まで**（§7.243）。
            # 枠だけで絞ると、大技の2発目が足りない戦いでは**小さい兵法の
            # 3発目・4発目**が枠を埋める（実測で「紀霊の【三尖刀】…157の損害」が
            # 同じ文で4回並んだ）。隊ごとに絞れば「誰の2発目か」が散る。
            rec = by_id.get(e.cast) or {}
            key = (rec.get("side", ""), rec.get("who", ""))
            if rep_by_unit.get(key, 0) >= LINE_REPEAT_UNIT:
                continue
            rep_by_unit[key] = rep_by_unit.get(key, 0) + 1
    ...
    mid = [e for e in kept if e.kind in ("兵法", "計略", "誘発", "苦戦", "壊滅")]
...
# 合成カード用の標準兵法。**実カードは使わない**（実編成のパネルは飽和する、§13）。
# 統制した条件で兵法の効果を測るために、全札に同じ兵法を持たせる。
SYNTH_SKILL = "＿標準兵法"
SYNTH_SKILL_POWER = 3.0
# 標準兵法の値段（§7.77 で定数→コスト一次に）。兵法の与える量は撃ち手の武力に
# 比例し、武力はコストで伸びるので、**兵法の出来高もコストに比例して伸びる**。
# 定数 1.20（§7.69）は中間帯の anchor としては働いたが、1〜2点では払い過ぎ
# （合成の能力値が 0.1 まで縮退し、実の安札が全部 +0.5 強く見えた）、8点超
# では払い足りなかった。釣り合いの支払い X を 2/3/5/8/10 点で二分法により
# 実測し、一次で当てはめた（2026-08-22・接敵抑制 0.60・兵法含の規則下）:
#   歩兵 X=0.88/1.28/1.74/2.17/2.53 → 0.63+0.195c
#   騎兵 X=0.96/1.10/1.57/2.23/2.56 → 0.52+0.207c
#   弓兵 X=0.60/0.74/1.05/1.33/1.60 → 0.38+0.122c（抑制で傾きが浅い）
SYNTH_SKILL_PAY = {"inf": (0.63, 0.195), "cav": (0.52, 0.207),
                   "arc": (0.38, 0.122)}
SYNTH_SKILL_KIND = "melee"
SYNTH_SKILL_TARGET = "敵1体（前衛の主力）"


def _synth(cost: float, typ: str, role: str = BAL) -> Card:
    """合成カード。SKILLS_ON のとき標準兵法を持ち、その値段ぶん能力値を下げる。

    **兵法を無料で配ると較正がずれる。** 兵法を持つぶん強いカードを「コストどおり」と
    ...
    if SKILLS_ON:
        ...
        # 同コストどうしの対戦では兵法の価値がコストに依らないことは実測済み
        # （effect_value の「測って却下」の注記）なので、定数でよい。
        a, b = SYNTH_SKILL_PAY[typ]
        return Card(cost, typ, role, skill=SYNTH_SKILL,
                    stat_cost=max(cost - (a + b * cost), 0.1))
...
# 実況の見本に使う対戦。**実カードで回す。** 兵法と固有特性が効いた場面を拾うのが
# 目的なので、合成カードでは意味がない（兵法も特性も持っていない）。
# 合計コストは両軍で揃える（差が付くと決着の理由が編成ではなく点差になる）。
NARRATE_CARDS = (
    ("赤壁", "呉", ["周瑜〔赤壁〕", "黄蓋〔苦肉〕", "魯粛〔榻上策〕",
                    "呂蒙〔白衣〕", "甘寧〔錦帆賊〕", "凌統〔公績〕"],
...
def cmd_formation(args) -> None:
    ...
    【⑧で訂正】かつて「ゲーム層は物理層と答えが逆になり、実カードでは健全な
    三すくみが出る」としていた（標準>広 73% など）が、**あれは性格2つ×固定
    シードの狭い計器が作った読み値**だった。陣形ごとにデッキを4種へ広げて
    デッキの質を分離すると、旧80人プールでも 広く浅い が推移的に強かった
    （魚鱗対鶴翼はほぼ五分）。いまは陣形ペアごとの残差を FORM_PAIR
    （コスト点）で相殺して、ゲーム層の各辺を狙いの帯へ寄せている
    （§7.53・§7.55。三すくみ自体は幾何とデッキ経済から自然に出る循環。
    FORM_BONUS は rank を配線した計器専用で、実戦の盤面では効いていない）。
    ...
    # 残差の相殺（FORM_PAIR）も切る。この計器の仕事は**生の幾何**を
    # 測ることで、相殺を入れたままだと相殺後の残差しか見えなくなる。
    keep_fs = dict(FORM_PAIR)
    FORM_PAIR.update({k: 0.0 for k in keep_fs})
    try:
    ...
    print("  分解: 推移 広-標 {:+.3f} / 標-深 {:+.3f}   循環 c {:+.3f}".format(
        e23 - c, e34 - c, c))
    print()
    print("  ※ 勝敗への影響はゲーム層（実カード・兵法あり・乱数あり）で測ること:")
...
def cmd_heal(args) -> None:
    ...
    **零点は「同じ兵法どうしで 0.00」。** 回復側とダメージ側で兵法が違うので、
    倍率を振って margin が 0 を切る点を探す。兵法が一度も撃たれない条件では
    ...
    def army(power, heal, typ=INF, gc=100.0):
        ...
        SKILL_TARGET[n] = "敵1体（前衛の主力）" if power > 0 else "味方1体（残兵力が最少）"
    ...
    print("回復とダメージの交換レート（ダメージ側は SKILL_SCALE = {:.1f}）".format(
        SKILL_SCALE))
    print()
    print("【計器】兵法が実際に撃たれているか。撃たれない条件では何を測っても嘘。")
...
def cmd_dot(args) -> None:
    """継続ダメージが「総量が同じ打ち切り兵法」より弱いことを確かめる。"""
    global SKILLS_ON
...
def cmd_stack(args) -> None:
    ...
    - **ally_skill 条件（鼓舞・呼応）は味方が兵法を撃たないと発動しない。**
      SKILLS_ON で測る（合成カードは標準兵法を持つ）。
    """
    global SKILLS_ON, TRAITS_ON
    from . import rosterdata as R
    R.load_skills_into_field()
...
def main() -> None:
    ...
    # **兵法をオンにすると相性表が変わる。** 兵法オフで較正した表を兵法オンの盤面へ
