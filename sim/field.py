...
- 兵種の三すくみ: 3方向が ±0.01 コスト点で揃っている（§2）。
...
■ トーナメント表を **推移成分**（単にどれかが強い）と **巡回成分**（本物の
   ...
   実測すると、補正 0 での偏りは推移 0.195 対 巡回 0.084 で、**ほとんどが単なる
   値付けのずれ**だった（三すくみが壊れていたのではない）。

■ 3兵種の総当たりで値付けし直した（`field.py price`）。歩兵基準の1本だけで
   合わせると残り1組のずれが取れないため、総当たりの推移成分を係数へ戻す。
   2回で収束した。

       反復0: 歩 +0.127  騎 -0.298  弓 +0.171
       反復2: 歩 +0.001  騎 -0.002  弓 +0.001   → 歩1.0000 / 騎1.0465 / 弓0.9097
...
■ 結果。単位はコスト点（総コスト30に対して）。

       兵種補正   歩→騎   騎→弓   弓→歩    巡回c    推移の幅
       0.000     -0.082  -0.081  -0.088  -0.0835    0.005
       0.020     +0.239  +0.235  +0.234  +0.2356    0.003
       0.040     +0.556  +0.547  +0.553  +0.5519    0.005
       0.060     +0.870  +0.856  +0.870  +0.8651    0.010
       0.080     +1.180  +1.161  +1.185  +1.1755    0.014

   **3方向が ±0.01 コスト点で揃った。** 推移の幅がほぼ 0 なので「単に強い兵種」は
   居ない。巡回成分は補正にきれいに比例する（c ≒ -0.084 + 15.7 × 補正）。
   時間刻みでも動かない（dt=1.0 / 0.5 / 0.25 で +0.556 / +0.552 / +0.551）。

   §5.3 の +4% を採ると、有利側の優位は **0.55 コスト点 ＝ 総コストの 1.8%**。
   カード1枚（コスト5）より十分小さく、編成で覆せる範囲にある。

■ 補正 0 の地点で巡回が **-0.084**（つまり素の幾何は 騎→歩→弓→騎 と逆回り）で
   あることは残しておく。補正はまずこれを打ち消してから効くので、補正 0.005
   あたりが実質の零点になる。

■ **仕様への提案**: §5.3 の目標帯を勝率（55〜80%）ではなく**コスト点**で書き直す
   こと。勝率の帯は決定論の盤面では踏めない。暫定案として「有利側の優位を
   総コストの 1〜3%（30点中 0.3〜0.9点）に収める」。

================================================================================
 3. 陣形（測れた。極端ではない）
================================================================================
面積 500×30 を保存したまま正面と奥行を交換する形で表した。正面を広げれば奥行が
薄くなる。同じ6枚で陣形だけ変えた総当たり（32戦/セル、値付けを直した後）:

       攻＼守          標準(前衛3)  広く浅い(前衛4)  狭く深い(前衛2)
       標準(前衛3)        50.0        12.5        56.2
       広く浅い(前衛4)      87.5        50.0        62.5
       狭く深い(前衛2)      43.8        37.5        50.0

- **0% / 100% のマスは1つもない。** 旧エンジンの 0/100 とは違う。
- 対角は厳密に 50.0、非対角は 87.5↔12.5 のように**反対称**（整合の検算として通る）。
- ただし**循環はしていない**。広く浅い > 標準 > 狭く深い の一直線である。
  「極端でない差」という目標は満たしたが、天敵のない型（広く浅い）が残っている。
- なお上は勝率なので、上記のとおり大きさではなく**符号のばらつき**を見ている。
  陣形もコスト点で測り直すべきで、それは未着手。

================================================================================
 残課題
================================================================================
 - **陣形をコスト点で測り直す。** 勝率では大きさが読めない（上の 2 を参照）。
   そのうえで循環を作る。いまは広く浅いが一方的に強い。
...
ACT_COEF = {INF: 1.0000, CAV: 1.0465, ARC: 0.9097}
...
def pair_cost_margin(ti: str, tj: str, slope: float, dt: float = 0.5,
                     rot: int = 4) -> float:
    """兵種 ti 対 tj の優位を**コスト点**で返す。

    左右を入れ替えて反対称化するので、左右の偏りは定義上打ち消える。
    勝率は使わない（下の cmd_triangle の注記を参照）。
    """
    tot = 0.0
    n = 0
    for r in range(rot):
        a = type_army(ti)
        a = Army(a.cards[r:] + a.cards[:r], a.form)
        for s in range(rot):
            b = type_army(tj)
            b = Army(b.cards[s:] + b.cards[:s], b.form)
            tot += (margin(a, b, dt) - margin(b, a, dt)) / 2.0
            n += 1
    return (tot / n) / slope


def type_table(dt: float = 0.5, rot: int = 4):
    """3兵種の総当たり表と、その推移成分・巡回成分。

    トーナメント表は「推移成分（単にどれかが強い）」と「巡回成分（本物の
    三すくみ）」に分解できる。**推移成分だけが値付けで消せる。** 巡回成分は
    値付けでは動かず、兵種補正がここへ効く。両者を分けずに勝率だけを見ていると、
    「三すくみが立たない」のか「1つが強いだけ」なのかが区別できない。
    """
    T = (INF, CAV, ARC)
    slope = yardstick(dt)
    M = {}
    for i in range(3):
        ...
        for j in range(i + 1, 3):
            m = pair_cost_margin(T[i], T[j], slope, dt, rot)
            M[(i, j)] = m
            M[(j, i)] = -m
    rating = [sum(M[(i, j)] for j in range(3)) / 3.0 for i in range(3)]
...
def main() -> None:
    ...
    s.add_argument("--dt", type=float, default=0.5)
    s.add_argument("--rot", type=int, default=4)
