...
================================================================================
 2. 兵種の三すくみ（途中まで。`field.py price` → `field.py triangle`）
================================================================================
■ 分かったこと: **兵種補正では直せない。三すくみを止めていたのは行動面の係数だった。**
...
■ この盤面で測り直した（`field.py price`）。兵種補正を 0 にして素の強さを測り、
   ずれをコスト点へ換算して係数へ戻す反復で、2回で収束した。

       反復0: 騎兵 +6.15%  弓兵 -9.90%
       反復1: 騎兵 -0.13%  弓兵 +0.84%
       反復2: 騎兵 +0.01%  弓兵 -0.03%      → 歩1.0000 / 騎1.0601 / 弓0.9085

■ 接敵抑制を入れた（§5.3「騎兵→弓兵: 高速接近により遠距離攻撃を封じる」）。
   これがないと弓兵は密着されても撃ち続け、騎兵→弓兵が 0.0% から動かない。
   ただし**効きが鋭い**。抑制 0.01 につき弓兵の値付けが約 0.6% 動く。
       抑制    0.00   0.15   0.30   0.45   0.60
       弓兵   -1.44  -9.90 -18.82 -28.18 -37.86  （歩兵基準の値付け %）
   0.15 を採り、ずれは上の係数で吸収した。

■ 測り直した後、三すくみは**はじめて兵種補正に反応するようになった**。
       兵種補正   歩→騎   騎→弓   弓→歩   同兵種
       0.000      68.8     6.2    37.5    50.0
       0.020      87.5    25.0    81.2    50.0
       0.040     100.0    62.5    87.5    50.0
       0.060     100.0    81.2   100.0    50.0

■ 残っている問題: **補正 0 の地点が 50/50/50 ではない**（68.8 / 6.2 / 37.5）。
   兵種補正は3方向へ同じだけ効くので、この偏りは補正では動かせない。3方向を同時に
   目標帯（55〜80%）へ入れるには、先に補正 0 の地点を 50 へ寄せる必要がある。
   偏りは射程・速度・接敵抑制という**構造の非対称**から出ており、§13 の
   「射程の効き方が相性ごとに非対称なため、兵種係数では直せない」と同じ場所である。
   **次に着手すべきはここ。** 値付け（`price`）は歩兵基準の1本しか見ていないので、
   3兵種の総当たりで値付けする形へ広げるのが筋だと思われる。

================================================================================
 3. 陣形（測れた。極端ではない）
================================================================================
面積 500×30 を保存したまま正面と奥行を交換する形で表した。正面を広げれば奥行が
薄くなる。同じ6枚で陣形だけ変えた総当たり（32戦/セル）:

       攻＼守          標準(前衛3)  広く浅い(前衛4)  狭く深い(前衛2)
       標準(前衛3)        50.0        18.8        56.2
       広く浅い(前衛4)      81.2        50.0        62.5
       狭く深い(前衛2)      43.8        37.5        50.0

- **0% / 100% のマスは1つもない。** 幅は 18.8〜81.2 で、旧エンジンの 0/100 とは違う。
- 対角は厳密に 50.0、非対角は 81.2↔18.8 のように**反対称**（整合の検算として通る）。
...
================================================================================
 残課題
================================================================================
 - 三すくみの補正 0 の地点を 50/50/50 へ寄せること（上の 2 を参照）。
 - 陣形の循環。いまは広く浅いが一方的に強い。
...
ACT_COEF = {INF: 1.0000, CAV: 1.0601, ARC: 0.9085}
...
def price_of(typ: str, dt: float = 0.5) -> float:
    """兵種の値付けのずれ。歩兵を基準に「総コスト30点中の何%か」で返す。

    勝率ではなく残存兵力率の差をコスト点へ換算して測る。勝率は飽和して
    0/100 になり、ずれの大きさが読めない。
    """
    base = flat_army()
    ...
    for r in range(6):
        a = type_army(typ)
        ms.append(margin(Army(a.cards[r:] + a.cards[:r], a.form),
                         type_army(INF), dt))
    ms.sort()
    return 100.0 * (ms[len(ms) // 2] / slope) / 30.0


def cmd_price(args) -> None:
    """行動面の係数をこの盤面で測り直す（§6.1 の値は旧エンジンのもの）。

    兵種補正を 0 にして素の強さを測り、ずれを係数へ戻す反復。
    """
    global ACT_COEF, TYPE_BONUS
    keep_b, keep_c = TYPE_BONUS, dict(ACT_COEF)
    TYPE_BONUS = 0.0
    ACT_COEF = {INF: 1.0, CAV: 1.0, ARC: 1.0}
    print("行動面の係数の測り直し（兵種補正 0、歩兵を基準）")
    print("  ずれが 0 になるまで係数へ戻す。0 にしてから三すくみを作る。")
    print()
    print("  {:<8}{:>10}{:>10}{:>12}{:>12}".format(
        "反復", "騎兵", "弓兵", "係数(騎)", "係数(弓)"))
    for it in range(4):
        pc, pa = price_of(CAV, args.dt), price_of(ARC, args.dt)
        print("  {:<8}{:>9.2f}%{:>9.2f}%{:>12.4f}{:>12.4f}".format(
            it, pc, pa, ACT_COEF[CAV], ACT_COEF[ARC]))
        if max(abs(pc), abs(pa)) < 0.6:
            break
        ACT_COEF = {INF: 1.0, CAV: ACT_COEF[CAV] * (1 + pc / 100.0),
                    ARC: ACT_COEF[ARC] * (1 + pa / 100.0)}
    print()
    print("  採用値: 歩 {:.4f} / 騎 {:.4f} / 弓 {:.4f}".format(
        ACT_COEF[INF], ACT_COEF[CAV], ACT_COEF[ARC]))
    TYPE_BONUS, ACT_COEF = keep_b, keep_c
