...
#   0.30    21.6%    16.7%     3/37
#
# 組ごとの幅（0〜96%）はどの値でも残る。札ごとに役割も技も違うので当然で、
...
# 1.6）が「後衛で守られて立つ弓」から技を奪い、純弓型がラダーで 39.7→28.6% へ
...
# 【§7.74 で 0.20 → 0.60 へ・技にも掛ける】「密着された弓は撃てない」を
# 実効にする。0.20 では取り付かれた弓の出力が74〜87%残り、改設計弓の武力
# （コスト比で騎兵の2〜4倍）が白兵でも勝ってしまい、カウンターが立たない
# （§7.73 の解剖）。0.60・技含で 束ね＞普通・騎兵＞束ね(77%)・普通＞騎兵 の
...
# 必殺技の直撃にも接敵抑制を掛けるか（§7.74）。弓の主砲は技なので、通常
# 射撃だけ抑えても弱点にならない（技外80%でも騎兵44%止まりの実測）。
SUPPRESS_SKILL = True
...
#
# 【§7.124】3.20 → 1.60 へ半減。城塞（壁＋後衛主砲）の技回転が「安全に立って
...
# （与ダメ給・被弾給）が無いと届かない。**安全圏の札ほど技が細るのは狙い。**
GAUGE_PER_SEC = 1.60
...
def _apply_skill(u: Unit, sk: "Skill", tstr: str, own, foe, t: float,
    ...
    if SUPPRESS_SKILL and u.typ == ARC:
        # 接敵抑制を技にも（§7.74）。矢数の減衰は掛けない — 技はゲージの
...
def simulate(a: Army, b: Army, dt: float = 0.25, t_max: float = T_MAX,
    ...
    **同じ編成どうしでも、種を入れれば結果は五分にならない。** それが乱数を
    ...
    # デッキ経済（前衛4枠の値段・弓枠の技価値）が作る「陣形どうしの実測の
...
# 標準技の値段（§7.77 で定数→コスト一次に）。技の与える量は撃ち手の武力に
# 比例し、武力はコストで伸びるので、**技の出来高もコストに比例して伸びる**。
...
# 実測し、一次で当てはめた（2026-08-22・接敵抑制 0.60・技含の規則下）:
