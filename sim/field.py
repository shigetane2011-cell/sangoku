...
零点（同じ編成どうし）は 0.00%。刻み dt=1.0 / 0.25 / 0.125 で答えが一致する
（REPULSE=2.0、SIDE_GAP=0）。決着は潰走で 480〜620秒。**ここまでは読める。**

  総コスト30固定・配分だけ動かす（相手は常に 5/5/5/5/5/5）
    均等 5/5/5/5/5/5      +0.00%   ← 零点
    第一線の2枚 7/3        +0.67%
    第一線の2枚 9/1        +4.67%
    第一線へ寄せる 9/9/9/1/1/1  +22.00%
    第二線へ寄せる 1/1/1/9/9/9  +6.00%

**現実的な配分（7/3・9/1）はほぼ加法的**になった。旧エンジンは 9/1 で
勝率17.9%（−32pt）だったので、桁が違う。極端な寄せ（+22%）は残っている。

残っている課題:
1. **戦列が横へ散る。** 斥力で押し合うため、開始 x=±500m が終了 x=±637m まで
   広がる。戦列を組むべきところで散っているので、隊列の維持が要る。
   斥力を弱めると散らないが、刻みで答えが変わる（不安定）。両立させる形が要る。
2. 三すくみが出ていない（-33 / -52 / +80%）。ただし1の解決待ち。
3. 陣形の比較も1の解決待ち。
...
・**斥力を「近づく成分だけ削る」形にする。** 押し返さないので散らないはずだった
  が、「近づいているか」の判定が硬い分岐になり、向きが接線に近いと刻みで
  切り替わる。7/3 が dt ごとに −1.3 / +19.8 / +21.1 / −10.0 と暴れた。
  **移動の規則に分岐を入れると刻みに依存する。** 距離だけで決まる連続な力にする。
...
SIDE_GAP = 0.0                            # 横の隙間。**0にして戦列を連続にする**
# 隙間を空けると、方陣は真正面の1枚としか接触できない。近接射程10mに対し
# 正面500mなので、40m空けるだけで「3つの独立した決闘」になる。
# **レーンを事故で再現していた。** 実際の戦列は連続しているので0にする。
START_GAP = 600.0                         # 開戦時の両軍前列の距離

MELEE = 10.0                              # 近接の届く距離（接触）
ZOC_FIELD = 150.0                         # 拘束圏を避け始める距離。滑らかに効く
REPULSE = 2.0                             # 斥力の強さ（引力を1としたとき）
# 0.5 や 1.0 では刻みを変えると答えの符号が反転する（7/3 が +1.3 ↔ -18.8）。
# 2.0 なら dt=1.0 / 0.25 / 0.125 で完全に一致する。**強いほうが安定する。**
BOW = 180.0                               # 弓の有効射程
...
def step_move(u, foes, dt):
    """標的へ寄る。**標的以外の敵の拘束圏は、滑らかに避ける。**

    拘束圏に入ると動けなくなるので、通り抜けようとせず横へ回る。
    **これだけで翼を回る動きが出る。** 迂回という規則も、その距離の定数も
    要らない。回り込みの長さは敵の正面幅から出る。

    **二値で判定してはいけない。** 最初は「一歩進んだ先が拘束圏なら横へずれる」
    と書いたが、刻みを細かくすると別の経路を選ぶため答えが変わった
    （dt=2.0 で −22%、dt=0.25 で +13% と符号が反転）。
    標的への引力と、標的以外からの斥力を足して向きを決める形にする。
    斥力は距離とともに連続に減るので、刻みを変えても経路が変わらない。
    """
    tgt = pick(u, foes)
    ...
    vx, vy = dx / n, dy / n
    for e in foes:
        if e.hp <= 0 or e is tgt:
            continue
        d = gap(u, e)
        if d >= ZOC_FIELD:
            continue
        # **足し合わせる斥力にする。分岐を入れないこと。**
        # 「近づく成分だけを削る」形も試したが、`近づいているか` の判定が
        # 硬い分岐になり、向きが接線に近いと刻みで切り替わって不安定になった
        # （7/3 が dt ごとに −1.3 / +19.8 / +21.1 / −10.0 と暴れる）。
        # 距離だけで決まる連続な力にすると、刻みを変えても経路が変わらない。
        w = REPULSE * (1.0 - d / ZOC_FIELD)
        ex, ey = u.x - e.x, u.y - e.y
...
def resolve(a_units, b_units, dt=0.25):
