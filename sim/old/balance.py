#!/usr/bin/env python3
# ============================================================================
# 【旧実装・保管のみ】このファイルは動かさないこと。測定にも使わないこと。
# ============================================================================
#
# レーン（3本の平行通路）と総大将（撤退したら即敗北）を前提にした v0.5 までの
# 実装である。どちらも設計判断として撤回済み。
#
#   レーン   … 三国志の戦場に3本の平行通路は不自然。位置ベース（連続 x,y）へ置換
#   総大将   … コストの加算性が壊れる原因だった。「残存20%割れで潰走」へ置換
#
# いまの実装は sim/field.py / sim/design.py / sim/rosterdata.py。
# **こちらで測った数値は現行エンジンと違う答えを返す。** 過去の記述の出どころを
# 確認するためだけに残してある。仕様書 §13 の「測定が嘘をついた」事例と同じ形で、
# 古い計器の数字を持ち出すのが一番危ない。
#
# 参照するなら docs/spec/basic-game-spec-v0.5.md の該当節と突き合わせること。
# ============================================================================
