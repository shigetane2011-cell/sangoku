...
# 直近この回数は同じ相手と当てない（§7.240 でテストプレイの決定により 3 → 12）。
# 3 のままだと「4人を順番に回る」だけで、毎時の天下では1日に同じ相手と6回当たる。
# 在野が36人いるので、12 にすると1日で13人・同じ相手は最多2回になる。
# 代償は噛み合いで、相手とのレート差の平均が 260 → 304 へ広がる（実測）。
REMATCH_GAP = 12
