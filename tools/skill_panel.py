...
#
# 値: 名簿15枚（兵種3 × コスト 2/4/6/8/10）を --yard 3 --real-base --bases 12 で測った
# 1点Δ残 の中央値（§7.188。§7.187 後の盤面）。
#
# **§7.188 以降、この旗はふつう要らない。** §7.184 でこれを入れたのは、札ごとの1点が
# 5.3倍に散っていて残存点が札ごとに別の単位になっていたからだった。§7.187 で矢数を
# 作り替えたら散らばりが **1.7倍**（0.0217〜0.0378）まで締まり、**残存点がそのまま
# 本番 BO3 の裁定に当たるようになった**: 高順 1.70 対 1.54（+10%）・関羽 1.36 対 1.34
# （+1%）・諸葛亮 3.01 対 3.39（−11%）・法正 0.96 対 1.41（−32%）。同じ4枚をこの共通の
