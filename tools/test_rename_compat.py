...
class PerformanceUnchanged(unittest.TestCase):
    ...
    EXPECT = {
        # §7.177 で内容を変えた（攻撃力 +5%（54秒）→ 攻防 +3%（90秒））。改名時の不変は 58a4fe5 で確認済み
        "榻上の策": ("味方全体", "攻撃力 +3%（90秒） + 防御力 +3%（90秒）", "150"),
        "逍遥津の奮戦": ("敵1体（正面）", "ダメージ 威力839%", "300"),
        "濡須の督戦": ("味方後衛", "攻撃力 +13%（60秒）", "150"),
        "捨て身の迎撃": ("敵1体（前衛の主力）", "ダメージ 威力950% + 反動 攻撃力 -10%（30秒）", "300"),
        "救主の奮戦": ("味方1体（残兵力が最少）", "回復 攻撃力の177%", "150"),
        "火船突入": ("敵1列", "継続ダメージ 威力45%（13秒）", "300"),
        "借風火攻": ("敵1列", "継続ダメージ 威力52%（14秒） + 命中率 -6%（30秒）", "300"),
        "陳倉の備え": ("味方全体", "兵法防御 +25%（54秒）", "150"),
        "護軍の迎撃": ("敵1体（正面）", "ダメージ 威力115%", "75"),
        "連営火攻": ("敵1列", "継続ダメージ 威力55%（14秒） + 移動速度 -30%（37秒）", "300"),
    }
