...
# 気勢（兵法ゲージの回り・§7.258）。**いま値札が無い**（design.mods_value に rate の
# 枝が無く、タダで買える）。移動速度が 0 だったのと同じ穴（§7.230）なので、
# 「量 × 秒 × 対象係数」で線形に置けるのか、それとも閾値の段なのかをここで見る。
# 量・秒・対象・段・車台をそれぞれ振って、**比が揃うかどうか**が判定。
add("rate", "気勢 +12%（40秒）", AA)          # 馬良〔白眉〕の案そのもの
add("rate", "気勢 +24%（40秒）", AA)          # 量を倍に
add("rate", "気勢 +12%（80秒）", AA)          # 秒を倍に
add("rate", "気勢 +12%（40秒）", AF); add("rate", "気勢 +12%（40秒）", AR)
add("rate", "気勢 +12%（40秒）", ME)
add("rate", "気勢 -12%（40秒）", EF); add("rate", "気勢 -12%（40秒）", EA)
add("rate", "気勢 +12%（40秒）", AA, "手数"); add("rate", "気勢 +12%（40秒）", AA, "大技")
add("rate", "気勢 +12%（40秒）", AA, "標準", 5.0, "arc")
add("rate", "気勢 +12%（40秒）", AA, "標準", 10.0)
# 足止め・混乱
...
# 返し討ち（通常攻撃反射・§7.262）。**新しい器なので単価が無い。**
# 量・秒・対象・段を振って、線形に置けるかを見る。
add("nrefl", "通常攻撃反射 +20%（40秒）", AF); add("nrefl", "通常攻撃反射 +40%（40秒）", AF)
add("nrefl", "通常攻撃反射 +20%（80秒）", AF); add("nrefl", "通常攻撃反射 +20%（40秒）", ME)
add("nrefl", "通常攻撃反射 +20%（40秒）", AA); add("nrefl", "通常攻撃反射 +20%（40秒）", AL)
add("nrefl", "通常攻撃反射 +20%（40秒）", AF, "手数"); add("nrefl", "通常攻撃反射 +20%（40秒）", AF, "大技")
add("nrefl", "通常攻撃反射 +20%（40秒）", AF, "標準", 5.0, "arc")
