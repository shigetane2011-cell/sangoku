...
したがって:
  ...
  - **札1枚ずつの帯調整は「帯 × 兵種」の中央と比べて決める。**
    **帯だけの中央と比べてはいけない**（§7.114）。帯の中央は騎兵の枚数で
    嵩上げされるので、歩兵の札を当てると騎兵の過大評価がそのまま
    「この歩兵は弱い」という読みに化ける。実際に踏んだ:

        帯だけの中央と比べて   張飛 -0.92 / 徐晃 -1.82  → 直す対象に見える
        帯 × 兵種の中央と比べて 張飛 ±0.00 / 徐晃 -0.90  → 張飛は直すところが無い

    8-10・均衡の6枚は 騎兵4枚が +2.55〜+3.00、歩兵2枚が -0.20/-0.27 と
    **兵種でまっぷたつに割れる**。役割の中の散らばりではなく兵種の段差である
...
def main():
    ...
    # ── 帯 × 兵種の中央から見た外れ値（**個別の帯調整はここを見る**）──
    #
    # **帯だけの中央を使ってはいけない。** この計器は騎兵を構造的に過大評価する
    # （上の注記）ので、帯の中央は騎兵の枚数で嵩上げされる。歩兵の札をその中央と
    # 比べると、**騎兵の過大評価をそのまま歩兵の「値上げ不足」として読む**ことに
    # なる（§7.114 で実際に踏んだ。帯だけで見ると張飛 -0.92・徐晃 -1.82 と出るが、
    # 兵種ごとに見れば ±0.00 と -0.90 で、張飛は直すところが無い）。
    #
    # 実測（2026-08-26）の 帯 × 兵種の中央: 騎兵 +1.65/+1.78/+2.76/+3.17 に対し
    # 歩兵 +0.06/-0.13/-0.09/+0.70・弓兵 +0.20/-0.02/+0.17/+0.94。**兵種で桁が違う。**
    med = {}
    for r in data:
        med.setdefault((band(r["cost"]), r["typ"]), []).append(r["avg"])
    import statistics
    med = {k: statistics.median(v) for k, v in med.items()}
    gap = sorted(((r["avg"] - med[(band(r["cost"]), r["typ"])], r) for r in data),
                 key=lambda x: x[0])
    print("\n── 帯 × 兵種の中央から見た外れ値（**個別の帯調整はここを見る**）──")
    print("  帯だけの中央と比べない。この計器は騎兵を過大評価するので、")
    print("  帯の中央は騎兵の枚数で嵩上げされる（§7.114）。")
    print("\n  下位10枚")
    for g, r in gap[:10]:
        print("  %+6.2f  (実測%+.2f) %-16s %s%.0f・%s" % (
            g, r["avg"], r["name"], r["typ"][0], r["cost"], r["role"]))
    print("\n  上位5枚")
    for g, r in gap[-5:][::-1]:
        print("  %+6.2f  (実測%+.2f) %-16s %s%.0f・%s" % (
            g, r["avg"], r["name"], r["typ"][0], r["cost"], r["role"]))
