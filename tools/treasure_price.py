...
DECK_CAP = {"d0": 34.0, "dcav": 32.0, "dcav2": 34.0, "dshu": 34.0,
            "dgo": 40.0, "dgunyu": 27.0}
...
DECKS = {
    ...
    # 【§7.203 以降は使っていない】騎兵の宝は dcav2 へ移した。土台として残して
    # あるのは、30点の検算と、比べたくなったときのため。
    "dcav":   FRONT + ["李典〔慎重〕", "曹洪〔救主〕", "貂蝉〔傾国〕"],
    # 【§7.203】騎兵の宝（赤兎馬・馬鎧）を **騎兵1枚（曹洪・コスト3）の盤**で
    # 測っていた。兵力+2% も相性の捻じりも、そんな小さい1枚では出番が無い。
    # 前衛を騎兵3枚（17点）へ組み替えた土台を足す。持ち手は曹操〔魏王〕
    # （コスト10・兵力11,272）。後衛は dcav と同じ李典・貂蝉を残す。
    "dcav2":  ["曹操〔魏王〕", "顔良〔河北の驍〕", "田豫〔国譲〕",
               "李典〔慎重〕", "貂蝉〔傾国〕", "樊建〔伝令〕"],
...
CONDS = {
    ...
    # 【§7.203】軽騎の鞍は馬上回避へ移ったので**騎兵に持たせないと何も起きない**
    # （郝昭＝歩兵のまま測って Δ勝率 ちょうど 0.00%・誤差も 0 と出た。
    #  「効かない」ではなく「効きようがない」を測っていた）。
    "dcav2_keiki":    ("dcav2", "t_keiki", "曹操〔魏王〕"),
    "d0_seino":       ("d0", "t_seino",     "曹仁〔堅守〕"),
    "d0_mokgyu":      ("d0", "t_mokgyu",    "李典〔慎重〕"),   # 矢を使う主砲の弓（§7.203）
    ...
    # 【§7.203】騎兵の宝は騎兵の厚い土台（dcav2）で、太い騎兵に持たせて測る。
    # 旧 dcav は騎兵が曹洪（コスト3）1枚しかなく、計器が空回りしていた。
    "dcav2_sekitoba": ("dcav2", "t_sekitoba", "曹操〔魏王〕"),
    "dcav2_bagai":    ("dcav2", "t_bagai",  "曹操〔魏王〕"),
