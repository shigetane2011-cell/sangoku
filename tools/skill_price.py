# -*- coding: utf-8 -*-
"""兵法1つの**盤面での値打ち**を測り、式が請求している額と並べる（§7.112）。

    python3 tools/skill_price.py 張飛〔当陽橋〕 張郃〔巧変〕
    python3 tools/skill_price.py --all           # 全札（重い）

なぜ要るか。`one_ruler` は「この札は帯の真ん中よりN点弱い」までは言えるが、
**どこで損をしているかは言わない**。能力値が薄いのか、兵法が高いのか、特性が
効いていないのかが分からないと、直しようがない。ここは兵法だけを取り出す。

**測り方は §7.5 と同じ通貨。** 同じ札を2つ用意し、片方から兵法だけを外して
差を取る。能力値・特性・槍・陣形はすべて同一なので、**差は兵法のぶんだけ**になる。
...
読み方:
    ...
    請求  … `design.effect_value` が能力値から引いている額（＝兵法の値札）
    実測  … 盤面で兵法が返している額
...
def _refund(g, want):
    """兵法を外し、その値札ぶんを能力値へ戻した同じ札（＝釣り合いの相手）。

    **能力値は設計式で引き直す。** `stat_cost` を上げるだけでは兵力しか動かない
    （武力・知力は CSV の値をそのまま持つ）ので、兵法代を返したことにならない。
    """
    import dataclasses
...
def measure(job):
    """兵法を「持つ札」と「値札ぶんを能力値へ戻した札」を突き合わせる。

    **これが値付けの検算そのもの。** プラスなら兵法のほうが得（安売り）、
    マイナスなら能力値のほうが得（払い過ぎ）。零なら値札が当たっている。

    兵法だけを外して**能力値はそのまま**にする測り方もあるが、それは「兵法の
...
def charged(g):
    """`design.effect_value` が請求している額（兵法のぶんだけ。特性・槍は除く）。"""
    from sim import design as D, field as F, rosterdata as R
    sk = F.SKILL_INFO.get(g["兵法"])
    if not sk:
        return 0.0
    return D.effect_value(sk, R._skill_target(g["兵法"]),
...
SWEEP_TARGETS = ("敵1体（前衛の主力）", "敵1列", "敵前衛", "敵後列")
...
def sweep_one(job):
    ...
    **能力値は下げない。** ここで測るのは「威力Pの兵法そのものの値打ち」で、
...
def do_zero():
    ...
    足した能力値が現金化されず、（能力値と突き合わせる）兵法側が相対的に得に
...
def main():
    ...
    if miss:
        ...
        return 1
    names = [n for n in names if rows[n]["兵法"]]

    jobs = [(n, f) for n in names for f in FORMS]
    print("測る: {} 枚 × {} 陣形（兵法だけを外した同じ札との差）\n".format(
    ...
    print("{:<16s}{:>4s} {:<12s}{:>7s}{:>7s}{:>8s}   {}".format(
        "武将", "消費", "兵法", "請求", "値打ち", "釣り合い", "釣り合い（鶴/魚/雁）"))
    out = []
    for n in sorted(names, key=lambda x: float(rows[x]["コスト"])):
