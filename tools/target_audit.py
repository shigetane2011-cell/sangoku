# -*- coding: utf-8 -*-
"""効果の向き先の監査（§7.93）: 兵法が**誰に当たっているか**を実測する。

    python3 tools/target_audit.py          # 兵法と固有特性を全部見る
...
やり方: 同じ合成カード6枚ずつの2軍を組み、A軍の1枚にその兵法だけを持たせて
...
盤面の規約（field.py の注記）はこうなっている:
  ...
  - 回復は対象の側へ。**つまり敵対象の兵法に回復を書くと敵が回復する。**
...
def run():
    ...
    rows = R.skills()
    bad, info = [], []
    print("兵法 {} 件を1発ずつ通して、動いた隊を数える。".format(len(rows)))
    for sk_row in rows:
        name = sk_row["兵法名"]
        target = sk_row["対象"]
    ...
    if bad:
        ...
        return 1
    print("向き先の食い違いは無し。**兵法も特性も、対象文字列どおりの側にだけ入っている。**")
    return 0
