...
| official24 | 24登録 | 在野そのもの（12性格 × 2人）。訓練集合 |
| special48 | 48登録 | 別の引き（12性格 × 3人）＋ 実験4型 × 3人。`validation`（盲検ではない） |
| final_blind | 0登録 | 次回リリース判定用の封印枠 |

**§7.195 で official24・special48 を今の名簿から全部組み直した。**
旧版は commit 8986fc8（2026-08-28）の凍結で、その後の再較正を1つも反映していなかった。
**過去の勝率と比べないこと。**
...
**この2つの集合は「盤面が壊れていないか」を測る力を持っていない。** 在野は
`sim/dummies.py` の生成器（兵種・役割・コストの重みで引く）で組むので、
探索で作った18人登録には構造的に勝てない — 組み直した直後でも赤チーム #1 に
official24 **100%**・special48 **97.9%** で負ける（§7.195 の実測）。
**「候補 対 在野」は「ふつうの編成がラダーをどれだけ抜けるか」であって、
釣り合いの指標ではない。** 釣り合いは 6マス表の辺・7型の総当たり・
個札の値札・赤チームどうしの直接対決で見ること。
